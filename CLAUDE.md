...
- `crates/cli` — `agentdocker`. `client.rs` talks the protocol, `format.rs` renders output, `mcp.rs` is the stdio MCP server (hand-rolled JSON-RPC; `Backend` trait lets tests run it against a mock instead of a daemon).
