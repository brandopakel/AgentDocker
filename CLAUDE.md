...
- `crates/ui-iced` — optional Iced design preview (`agentdocker-design`, requires `--features preview`). Fictional in-memory data; not a production replacement. See `docs/ICED-DESIGN.md` for the design decisions and migration gates. Validate preview changes with its explicit feature enabled; normal workspace builds omit this binary.
