...
pub(crate) async fn serve(launch: Launch) -> anyhow::Result<i32> {
    ...
    let _owner_lock = lock::try_exclusive(&socket.with_extension("lock"))
        .context("cannot lock the session")?
        .context("another session owner holds this agent")?;
...
async fn supervise(
    ...
    tasks.spawn(write_log(log, sink));
...
async fn finish(
    ...
    match serde_json::to_vec(&report) {
        Ok(bytes) => {
            if let Err(error) =
                std::fs::write(&staged, bytes).and_then(|()| std::fs::rename(&staged, &path))
            {
    ...
    loop {
        tokio::select! {
            line = reader.next_line() => {
                ...
                let command: OwnerCommand = match serde_json::from_str(&line) {
                    Ok(command) => command,
                    Err(_) => continue,
                };
                if !current() {
                ...
                match command {
                    ...
                    OwnerCommand::Input { bytes } => {
                        // Dispatched without an await after the epoch check,
                        // so a takeover cannot slip a stale keystroke in. A
                        // full queue is refused and said, never held.
                        if let Some(input) = &shared.input
                            && input.try_send(bytes).is_err()
                        {
                            send(&mut writer, &OwnerReport::InputDropped).await?;
                        }
                    }
                    ...
                    OwnerCommand::Attach { after } => {
                        // The subscription already exists, so nothing falls
                        // between the retained bytes and the live ones;
                        // duplicates are cut by offset below.
                        let (start, bytes) = {
