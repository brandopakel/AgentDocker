...
- **Linux:** the CLI and daemon are exercised in CI on x86-64 and ARM64 and the desktop app builds and packages there, but no release has been tried on a real distribution by a person yet. **Windows:** the merged daemon/CLI foundation answers over native named pipes. Replacement PR #214 (superseding #210) passed 284 native core/host tests and 50 daemon/CLI/desktop checks, including ConPTY, owner lifetime, database crash recovery and fresh-home window startup. Broader graphical/input, service, installer and real-provider acceptance remain, and no Windows desktop download is available. See the [Windows port status](docs/WINDOWS-PORT.md).
