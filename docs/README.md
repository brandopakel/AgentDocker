...
| [Windows port](WINDOWS-PORT.md) | Daemon/CLI foundation complete in #206. Open #210 implements ConPTY/session ownership: native `dcceff2d` passed terminal, owner-death and input-backpressure checks, then failed SQLite WAL ownership during crash recovery. Fix `82bb2cd6` is under native validation. Full managed-session, desktop, service, installer and real-provider acceptance remain; no Windows desktop download is available. |
