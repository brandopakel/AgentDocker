...
`f5e298f4` carries them. In review: the board of work (#176), persisted drafts (#178) and
observation-preserving session resumption (#179). Remaining acceptance is not an
...
| [Remaining work](REMAINING-WORK.md) | Partial: current disposition of existing engineering, acceptance and release requirements, kept current with each merge; the September 17 rows say what is installed (`f5e298f4`), what is in review (#176, #178) and what acceptance is still owed. |
...
| [Remote connector](REMOTE-CONNECTOR.md) | Partial: `agentdocker connector` is in source with unit tests and a loopback real-binary trial; acceptance against an actual Claude or ChatGPT account, the tunnel helper, egress allowlisting, CIMD and desktop status remain. |
...
| [Architecture](ARCHITECTURE.md) | Partial: protocol/phase inventory (delivery rows 1–36, of which 17 federation, 20 Windows, 28 reload, 34 usage, 35 board and 36 drafts are open), journal/checkpoint maintenance and an environment-free core exist; live replacement is in source behind `AGENTDOCKER_EXPERIMENTAL_RELOAD` with provider-side acceptance still ahead, and Windows is incomplete. The usage collector/protocol/CLI/screen and federation remain proposals. |
...
| [Desktop UX](DESKTOP-UX.md) | Implemented home/Tools simplification and capability evidence. The Messages workspace (#150/#152/#160/#163/#170), the project header's Pause…/Resume (#173), notification clicks that reveal an archived message (#175) and the channel-capable Claude launch (#177) are merged and installed in `f5e298f4`. The Board tab (#176) and persisted drafts (#178) are described as in review. Physical Enter/IME and broader installed-candidate acceptance remain. |
...
| [Guided setup](GUIDED-SETUP.md) | Implemented preview/apply/undo, configuration locks, per-session contact/input evidence and, since #177, a channel-capable Claude MCP entry (`--claude-channel`) that setup writes and health recognises; the actual user entry was updated on September 17 without a provider restart. Installed-candidate acceptance of a fresh channel session remains. |
| [Activity and messaging](ACTIVITY-AND-MESSAGING.md) | Partial: activity/hooks and opt-in input adapters exist; ordinary hooks alone cannot wake idle models — reconfirmed on September 17, when a plain Claude session received the person's channel message only with its next prompt. |
| [Message delivery audit](MESSAGE-DELIVERY-AUDIT.md) | September 16 live Claude failure confirmed: twelve messages waited for a human prompt. Launch defaults and composer readiness are implemented and have bounded rendered acceptance. Owned Codex active-turn input is merged in PR #162 with bounded actual-client acceptance. Installed standalone-terminal bounded active input passed on September 17 after the reviewed receiver replacement: fresh CLI human-route and actual Claude peer inputs entered the same live turn. Existing-session reconnect and broader provider acceptance remain open. Implemented durable shared queue and exact receipts. PR #131 adds provider availability, quota isolation, queue gating and recovery, with actual isolated Claude/Codex trials, 135 runtime/interruption cases and [184 native steps at `66c4247`](https://github.com/brandopakel/AgentDocker/pull/131#issuecomment-5673268646). Combined `ec45cea` also passed mid-tool, pending-denial and same-identity controller replacement cases. Broader reviews, provider/account-reset and sustained acceptance remain. |
| [Codex input](CODEX-INPUT.md) | Partial: managed bridge and existing-terminal native queue implemented; six actual Codex 0.154.0 release trials and the 962-Rust/70-Python gate passed at `7133023`. PR #148 is merged; one installed existing-session auto-bootstrap, peer wake and exact receipt passed. The September 17 installed receiver upgrade also passed bounded active-turn CLI/peer input with exact receipts and preserved provider sessions. Zero-prompt startup/reopen, broader elicitation/secret input and historical reconciliation remain. |
| [Claude channel input](CLAUDE-CHANNEL-INPUT.md) | Partial: explicit channel input and question queue implemented; #164 folds a returning session into its record and #177 makes setup write the channel form. Relaunching the existing plain session with the channel, its consent, and idle receipt in that session remain. |
| [Notification routing audit](NOTIFICATION-ROUTING-AUDIT.md) | Bounded installed acceptance passed: approved OS permission, automatic daemon posting, real foreground/background message/question/stale-target clicks, preserved draft and cold private-origin routing. The person's September 17 finding — a click while the conversation was already open did nothing — is fixed in #175 (installed): the archived row is marked and scrolled to, paging back a bounded number of pages, and an actual Notification Center press on the installed revision with the app hidden revealed the exact message (September 17). Zero-process cold launch, a physical mouse click, the page-back path on a real click and signed-release acceptance remain. |
| [Identity repair](IDENTITY-REPAIR.md) | Partial: offline preview/apply implemented and tested; production reconciliation not done (it needs a quiet window with every recorded session ended). Same-session reconnect is separate (#164); #179 (in review) carries retained observations across the fold. |
...
| [Native delivery](NATIVE-DELIVERY.md) | Partial: merged implementation record; installation complete on this Mac (reviewed `f5e298f4`), retention and reload in source, broader release acceptance incomplete. |
| [Local build](LOCAL-BUILD.md) | Reference: build, Applications installation, compatibility paths, rollback and the two-way `install.sh`/managed-install protection implemented; the backed-up coordinator switch to verified `cf64ca3` passed; the latest backed-up `f5e298f4` schema 23 activation (September 17) preserved both live provider identities and every retained receipt; an installation asks a running daemon to reload to the activated release and reports the answer, and until the gate is on the switch remains a quiet-window restart. |
