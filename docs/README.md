...
and session reconnect #164. September 17 installed `79981beb` includes the
reviewed messaging controls. Remaining acceptance is not an open implementation claim.
...
| [Desktop UX](DESKTOP-UX.md) | Implemented home/Tools simplification and capability evidence. PR #150's Messages workspace, archive, read state, threads and drafts are merged after review and CI. PR #152's final UI fixes passed 1007 Rust tests, 77 Python checks and 275 native workflow steps, then merged after final CI/review. PR #170 adds installed New DM/channel, invitations, scoped mentions and submit actions. The actual form opened; physical Enter/IME and broader installed-candidate acceptance remain. |
...
| [Notification routing audit](NOTIFICATION-ROUTING-AUDIT.md) | Bounded installed acceptance passed: approved OS permission, automatic daemon posting, real foreground/background message/question/stale-target clicks, preserved draft and cold private-origin routing. Fully closed-app, broader variants and signed-release acceptance remain. |
...
| [Local build](LOCAL-BUILD.md) | Reference: build, Applications installation, compatibility paths, rollback and the two-way `install.sh`/managed-install protection implemented; the backed-up coordinator switch to verified `cf64ca3` passed; the latest backed-up `79981beb` schema22 activation preserved all four external provider identities; an installation asks a running daemon to reload to the activated release and reports the answer, and until the gate is on the switch remains a quiet-window restart. |
