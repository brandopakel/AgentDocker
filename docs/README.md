...
| [Codex input](CODEX-INPUT.md) | Implemented managed bridge and existing-terminal native queue. Installed active peer-answer stall is fixed in merged #202; idle and active trials retain exact source/receipt evidence. September 21 retained-hook recovery and root/child isolation passed the native trial and are installed as local preview `ff4efc61`; final integration remains separate. A 65-second busy turn passed, but zero-prompt reopening failed twice with no resume hook/receiver. Zero-prompt startup/reopen, wider permissions/elicitation, throughput and historical reconciliation remain. |
