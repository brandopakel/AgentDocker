...
`f5e298f4` carries them. In review: the board of work (#176) and persisted drafts
(#178). Remaining acceptance is not an open implementation claim.
...
| [Identity repair](IDENTITY-REPAIR.md) | Partial: offline preview/apply implemented and tested; production reconciliation not done (it needs a quiet window with every recorded session ended). Same-session reconnect is separate (#164) and is being extended to carry retained observations. |
