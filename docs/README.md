...
| [Local build](LOCAL-BUILD.md) | Reference: build, Applications installation, compatibility paths, rollback and the two-way `install.sh`/managed-install protection implemented; the backed-up coordinator switch to verified `cf64ca3` passed; Claude's subsequent `72b1eb4` schema21 installation is now observed serving on this Mac with all four external provider identities preserved; an installation asks a running daemon to reload to the activated release and reports the answer, and until the gate is on the switch remains a quiet-window restart. |
...
| [2026-09-17-pause-rollback-refusal.json](verification/2026-09-17-pause-rollback-refusal.json) | passed |
