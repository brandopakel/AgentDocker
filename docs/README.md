...
`f5e298f4` carries them. Persisted message drafts (#178) are merged and await
installation. The board (#176) and preserved resume state (#179) have also merged after
final CI/review. Startup ordering (#180) has a clean review and awaits CI on
its merge resolution; the combined candidate is packaged but not installed. Remaining acceptance is not an
...
| [Remaining work](REMAINING-WORK.md) | Partial: current disposition of existing engineering, acceptance and release requirements, kept current with each merge; the September 17 rows say what is installed (`f5e298f4`), what is merged but not installed (#176, #178, #179), what awaits final CI/integration (#180) and what acceptance is still owed. |
...
| [Desktop UX](DESKTOP-UX.md) | Implemented home/Tools simplification and capability evidence. The Messages workspace (#150/#152/#160/#163/#170), the project header's Pause…/Resume (#173), notification clicks that reveal an archived message (#175) and the channel-capable Claude launch (#177) are merged and installed in `f5e298f4`. The Board tab (#176) is merged but not installed; persisted message drafts (#178) are merged but not installed. Physical Enter/IME and broader installed-candidate acceptance remain. |
