...
| T06 | Bounded fuzz campaigns | Protocol/resource/path decoding, metadata parsers and token filters. Current scheduled targets cover only part of this scope. Retain crashes and minimized regressions with nightly/source identity. | Partial: scheduled protocol/resource-key fuzzing passed on main September 13–14. That does not close broader metadata/token-parser campaigns; retain crash/minimization evidence. |
