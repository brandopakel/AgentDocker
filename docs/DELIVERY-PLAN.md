...
Replacement Windows PR #214 supersedes #210 and remains open for integration. Native `cadf3d58` and final code `e3ada9db` each passed 284 core/host
...
record 871 Rust tests, 65 Python checks, 218 native steps, 12 installation
scenarios, Launch Services, actual Claude sender identity and a short actual
Codex queue/reconnect trial at reviewed `d70668c`. Main's source inputs and
pre-package binaries match that reviewed build. The earlier
...
are superseded by this audit and [Remaining work](REMAINING-WORK.md); original
failures, exact source identities and unresolved acceptance stay intact.
...
active delivery. PR #169 implements a native hook handoff in the existing
...
installed acceptance and the remaining provider cases pass. Source `5545697`
passed the actual Codex 0.154.0/local-model busy trial: peer, human project and
global messages entered one active turn within 8.7 seconds with exact receipts.
Dropped hook output retained the queued IDs without replay. The 1,093-Rust/84-Python
gate passed (seven skipped). The existing receiver still needs a safe upgrade
from its pinned older release; app installation alone does not change it.
See the [existing evidence](verification/2026-09-15-native-codex-queue.json).
...
| 5. Deliver platform parity | Finish native Windows host/IPC/process/terminal/service/path/installer adapters and runtime CI. Complete Linux desktop inventory/packages and target-distribution GUI/service tests. Repeat the same semantic tests on each OS. | Four Mac/Linux desktop CI targets and Windows core/host/named-pipe foundations pass on main. Replacement #214 and its predecessor #210 passed 284 native tests and 50 daemon/CLI/desktop steps, including ConPTY/lifetime, SQLite crash recovery and fresh-home window startup. Target-distribution/service, second-Mac/physical-Intel and full native Windows daemon/GUI/service/installer acceptance remain; no Windows desktop download is published. |
...
4. Check any new overlapping branches against current main before combining them. The historical #45–#54 stacks are already integrated. Preserve schema meaning, startup readiness, lease lifetime, native IPC compatibility and strict test-runner settings during conflict resolution.
...
|---|---|---|---|
| T01 | Standard gate: formatting, strict Clippy, nextest, separate doctests, installer/package checks, release build | Exact-head local and CI records. Zero retries, flaky failure and 500 ms leak failure remain. The documented Darwin runner workaround must not mask child leaks. | Passed: main standard, coverage, desktop, engine and Windows-foundation CI. Reviewed `d70668c` local gate: 871 Rust / 65 Python, lint, doctests and packaging. New source still requires its own gate; zero retries and leak failure remain. |
| T02 | Deterministic correctness and meaningful Proptest reference models | Claim/renew/release/expiry/finish, physical aliases, cancellation, deadlock/fairness, stopping writers, stale read sets, durable ordering and image/source-bound validation. Audit model coverage and commit minimized failure seeds. | Partial: deterministic and property regressions pass in main CI; all requested reference-model scenarios are not yet evidenced. Preserve physical-alias, fairness, cancellation and minimized failure cases. |
| T03 | Coverage-directed tests | Inspect cancellation, authentication/revocation, migration, restore and error/cleanup branches. Link LCOV and actual added scenarios; coverage percentage alone cannot close a gap. | Partial: main coverage CI produces LCOV; closure still requires inspecting the relevant missing error/cancellation/migration branches, not a percentage alone. |
| T04 | Criterion and private Bencher comparison | Lease, fingerprint, SQLite write and recovery-query benchmarks against the actual base with verified manifests. Separate machines/engines as testbeds; no GitHub credential setup. Review trend variance before blocking thresholds. | Partial: main Performance CI and source-bound private comparisons exist. The [100-client release-response failure](verification/2026-09-07-integration-benchmark-failure.json) remains undiagnosed; repeated platform thresholds are not established. |
| T05 | Native IPC contention and load | 1/10/100 clients; disconnects, slow readers, full pipes, contention, restart and handoff. Existing contention/release benchmark includes connection setup. Add missing dedicated stale/restart latency workloads and Windows named-pipe equivalents. | Partial: native contention, queue/reconnect and slow-reader fixtures exist. Dedicated stale/restart latency, saturated connection establishment and full Windows transport workloads remain. |
| T06 | Bounded fuzz campaigns | Protocol/resource/path decoding, metadata parsers and token filters. The scheduled targets cover protocol, resource keys, engine metadata and token filtering. Retain crashes and minimized regressions with nightly/source identity. | Passed bounded scope: scheduled protocol, resource-key, engine-metadata and token-filter campaigns passed on main September 13–14, 120 seconds per target. Nightly/source manifests and failures are retained; this is not indefinite fuzz coverage. |
| T07 | Concurrency model selection | Inspect new synchronization algorithms; use Loom only if an extracted in-memory algorithm warrants it. SQLite/OS ordering needs real integration fault tests. k6 remains conditional on an actual supported network transport. | Reviewed/conditional: one daemon state mutex remains. No additional Loom or k6 campaign is required absent a suitable new in-memory algorithm or supported network transport; real OS/SQLite fault tests remain T09. |
...
| ID / stage | Scenario | Existing acceptance requirements | Audited status and remaining acceptance |
|---|---|---|---|
