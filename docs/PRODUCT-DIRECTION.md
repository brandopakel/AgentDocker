...
5. Delivered in #45/#47/#50: admission policy and quotas, restart policies with backoff and dependencies, the daemon descriptor handoff behind `daemon reload`, the daemon-attributed `commit`, and rtk-compressed views of retained logs. What still wants doing here is watching them under real use rather than building more: the combined implementation has not yet been exercised against a restore, an installation and an upgrade in one sitting.
