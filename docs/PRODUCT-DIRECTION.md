...
|---|---|---|
| macOS arm64/Intel | `scripts/bundle-macos.sh` builds `AgentDocker.app` — Info.plist, an `.icns` drawn by `scripts/icon.py` on Apple's 824-of-1024 grid — the release archives carry it, `install.sh` puts it in `~/Applications`, and `agentdocker ui` prefers it. DMG packaging and explicit per-user installation, activation and rollback are implemented, with Apple Silicon and Rosetta graphical trials recorded. The bundle is **ad-hoc signed only** (`codesign --sign -`) | Developer ID signing, notarization and stapling — without them Gatekeeper refuses the app on any Mac but the one that built it, so the download is not usable by anyone else yet. Then release publication, Intel hardware and longer upgrade trials |
| Linux x86-64/ARM64 | Released CLI/daemon archives; the desktop app builds from source and has a native archive and launcher, with x86-64 Xvfb/Mesa graphical CI and a per-user installer accepted in CI | Distribution packages, desktop inventory, ARM64 graphical trials, and runtime/notification/service acceptance on target distributions |
