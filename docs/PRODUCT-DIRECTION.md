...
Main contains a per-user daemon, CLI, native GUI, persistent registry/inboxes/events, project grouping, native process supervision, expiring physical-path leases, FIFO waiting/deadlock detection, activity derived from observed coordination, and messaging. Working-state features include content observations, stale-context checks, a change ledger, journal/digests, checkpoints, validation evidence, linked worktrees, integration previews and addressed/exported handoffs. Channels and contests coordinate reviews and competing attempts. Since September 15: a Messages workspace (archived conversations, named channels, threads, search, read cursors, mentions, new DM/channel from the app), native notification routing to the message, a project pause the person sets and lifts (`pause`, refusing agents' new leases with the reason), same-session reconnect of a provider that comes back, and a bounded reader of local Codex/Claude usage logs. The board of work (cards with acceptance text, pulled once over a `task:<id>` lease, #176) and persisted message drafts (#178) are merged and installed. Question and Board drafts landed through #191; initial usage collection/CLI/GUI landed through #194 and still needs activation on the serving coordinator.

Discovery runs in the daemon every five seconds. Installed-tool inventory includes CLI paths/versions, selected macOS application bundles and known configuration wiring. Setup supports selected MCP hosts and Claude Code hooks with dry-run output and backups. The GUI includes agents, runtimes, journal, leases, events, human questions, a terminal and a CLI console. Notifications are best effort through installed OS tools. PTY attach/detach and opt-in command relaunch exist. Daemon handover is implemented behind the experimental reload gate; broader acceptance remains open and ordinary gate-off reload still refuses. Multiplexer adapters recognize reported/observed sessions and can launch a tmux-owned agent.
...
| Linux | Native desktop archive/launcher and ARM64/x86-64 graphical/package CI; per-user installer implemented | Target-distribution graphical/service/package acceptance |
| Windows | Core/host foundations, the named-pipe transport (`crates/host/src/ipc/windows.rs`) and native CI for those crates. No Windows service adapter, daemon path or ConPTY supervision exists; the full product still depends on Unix APIs | Complete process/IPC acceptance, ConPTY, service/session lifecycle, physical path semantics, installer and full native daemon/GUI CI |
| GitHub and shell installer | [v0.1.0](https://github.com/brandopakel/AgentDocker/releases/tag/v0.1.0), four archives and checksums; `install.sh` selects a target | Tag v1.0.0 on main once the open integrations land — the [road to v1](REMAINING-WORK.md#the-road-to-v1-a-release-other-people-can-install-and-try); the workflow builds the archives and formula from the protected tag |
| Homebrew | `packaging/homebrew/generate.py` writes a formula from the release's real per-target checksums, CI validates it with `ruby -c` and uploads it as a release asset. The [tap](https://github.com/brandopakel/homebrew-tap) contains the v0.1.0 formula; repository publishing configuration is present (verified September 9) | Verify the next release updates the formula and publishes the generated cask (`packaging/homebrew/agentdocker-app.rb.in`) with a notarized app; the tap currently has no app cask |
...
The published release is source `52fd88d`, before sessions, human questions/notifications, fair waiting/activity, contests, multiplexer work, the messaging workspace, the board and the September landscape items; v1 is the next tag and the first release that carries them. A source build or local preview must name its exact revision; package version `0.1.0` alone cannot distinguish these builds.
...
1. ✅ Fix the confirmed restore readiness/persistence defects and harden private state/log creation. Turn the fault probes into regression tests. Keep automatic restore off in the initial trial. (Done: merged #46 and follow-ups; see [native delivery](NATIVE-DELIVERY.md).)
2. ✅ Run a pinned native trial on this Mac in a disposable repository/private state directory. Exercise the GUI and protocol before configuring one test Claude Code session and one test Codex session. Preview configuration changes and verify message/observation round trips. (Done: the isolated, native and provider stages of the [local trial](LOCAL-TRIAL.md) ran; this Mac now runs reviewed release `f5e298f4` in ordinary use, with the sustained/second-Mac stages still open.)
3. ✅ Desktop onboarding, per-tool capability/health reporting, undo and packaging are in main (guided setup with preview/apply/undo, runtime inventory, the update consumer, installer retention and rollback); what remains of this step is distribution: public macOS desktop distribution needs Developer ID signing and notarization of the native `agentdocker` bundle, and the proposal is that v1 ships unsigned with the Gatekeeper note in the README until an identity exists (the maintainer's call). The download/update consumer, feed generation and installer uninstall/retention exist; complete signed hosted publication and maintained distribution channels, including the app cask. Source builds and explicit local previews remain separate from a supported signed release. Publish the tested candidate and run the independent second-Mac trial. Keep Bencher credentials outside the repository and GitHub; upload verified benchmark artifacts using private configuration.
...
5. Audit and harden the merged admission/quotas, restart/backoff/dependencies, retention, daemon-attributed commit and bounded log implementations. The delivery audit reproduced policy-loading and run-admission defects; their fixes are merged with integrated verification. Automatic journal/checkpoint maintenance is implemented (`[journal] retention`, `journal prune`, `checkpoints prune`, `daemon vacuum`), with sustained acceptance still open. Live daemon replacement is in progress: session owners, the coordinator fence and the successor handover are in source, and `daemon reload` hands over behind `AGENTDOCKER_EXPERIMENTAL_RELOAD=1` while the acceptance in [LIVE-DAEMON-UPGRADES.md](LIVE-DAEMON-UPGRADES.md) is recorded; without the gate it returns unavailable, as #59 made it after actual batch/PTY termination and log-loss failures. Connected-client resumption and installation integration are also in source with private acceptance evidence; broader provider, attached-terminal, cross-version and platform acceptance remains open. Merged code alone does not satisfy that acceptance.
