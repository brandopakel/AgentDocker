...
|---|---|---|
| macOS arm64/Intel | v0.1.0 archives contain CLI, daemon and GUI executables. `scripts/bundle-macos.sh` builds `AgentDocker.app` — Info.plist, an `.icns` drawn by `scripts/icon.py` on Apple's 824-of-1024 grid — the release archives carry it, `install.sh` puts it in `~/Applications`, and `agentdocker ui` prefers it. The bundle is **ad-hoc signed only** (`codesign --sign -`) | Developer ID signing, notarization and stapling — without them Gatekeeper refuses the app on any Mac but the one that built it, so the download is not usable by anyone else yet. Then DMG packaging, upgrade/rollback and Intel runtime trials |
