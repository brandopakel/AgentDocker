...
Validation and checkpoint records bind the immutable image, retained build, engine connection, network, UID and explicit environment. Recovery reports `environment_matches`, refuses acknowledgement after an environment change, and shows passing validation only while both checkout and environment match. Integration requires validation from the target's environment as well. Native evidence remains distinct from container evidence. Schema 5 stops older daemons from ignoring these bindings or validation deadlines. The existing ignore-aware content scope and point-in-time limitations still apply.
