...
| 7 (implemented) | GitHub CI and CodeRabbit review | Shared contract tests plus real Docker and Podman Linux jobs; macOS VM checks documented separately; findings resolved before integration |

Implemented on main and included in v0.1.0: scoped authentication for externally managed containers, physical path mapping, worktree integration, image build adapters and managed launch/stop/restart recovery are implemented. Opt-in checkout/endpoint mounts, macOS Podman scoped bridges, and image-bound validation are implemented. Docker Desktop uses a private engine-volume socket relay; actual macOS Desktop evidence is tracked separately from Linux Docker. Podman has been installed on the development host for real integration testing. Engine-specific results must be reported individually; Docker compatibility must not be inferred solely from a Podman pass.
...
Local rootless Podman uses `keep-id`; rootful Docker uses the daemon user's UID/GID, and rootless Docker uses container UID 0, which maps to its host user. Remote Docker endpoints and Docker `userns-remap` are rejected. A macOS Docker context must report Docker Desktop and pass actual shared-file/UID probes. Shared workspace/socket containers disable SELinux labeling because host and SSH socket processes have different labels; they retain UID isolation, dropped capabilities and `no-new-privileges`. Inspection verifies the exact bind sources, destinations, read/write policy, user, workdir and network mode before lifecycle operations. Paths must fit Unix socket limits and cannot contain mount-option delimiters.
...
Validation and checkpoint records bind immutable image and build input evidence, engine, network, UID and explicit environment; local build IDs and engine connection names remain audit metadata. Recovery reports `environment_matches`, refuses acknowledgement after an environment change, and shows passing validation only while both checkout and environment match. Integration requires validation from the target's environment as well. Native evidence remains distinct from container evidence. Schema 7 stops older daemons from ignoring these bindings or validation deadlines. The existing ignore-aware content scope and point-in-time limitations still apply.
...
### Engine-volume socket relay

Docker Desktop automatically selects the relay for a mounted run. `--engine-relay` selects the same transport explicitly on Linux Docker or rootless Podman (with `--podman-machine NAME` on a Podman VM). The agent keeps a private Unix endpoint and its selected network policy, including `none`. A separate helper owns the socket in a labeled engine volume, mounted read-only by the agent. The helper has no host mounts or networking, runs with the workspace UID, drops capabilities, and disables engine logging. It forwards bounded authentication/request frames over the attached engine CLI streams to the host's restricted Unix endpoint. Credentials are never CLI arguments, registry values or helper logs. There is no TCP listener or engine-socket mount.

The helper image is built from an embedded relay and a digest-pinned Python base; its complete build provenance is retained with the run. Preparation verifies host share visibility and effective UID, and readiness inspection verifies the helper's image, labels, volume, user, network and logging policy before writer creation. The relay admits at most 32 connections, at most 1 MiB per protocol frame, with a 30-second request lifetime and bounded queues. A failed stream is rebuilt by exact owned helper identity without restarting the agent writer; recovery never interprets helper exit as writer exit.
