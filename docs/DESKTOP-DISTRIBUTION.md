...
Use a fresh output directory for every package or trial. `native-build.json` records the exact source input hash, commit, tree, dirty status, compiler, target and executable checksums. Cargo reports each artifact path, so target-directory overrides are preserved; use the reported `binary_directory` when packaging. Packaging refuses changed binaries, source/version mismatches, wrong architectures, mixed universal inputs and existing output directories. The final manifest hashes the signed executables and archives. A failed build or signer does not publish a completed artifact directory. A checksum detects corruption; it is not publisher authentication.
