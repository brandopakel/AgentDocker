...
| `connector enable <serve arguments> [--dry-run]` | Install/start a new login service or start its byte-identical definition. Exclusive file creation and exact comparison refuse symlinks or different existing settings without replacing them. The desktop uses this conservative entry point; `install` remains the explicit replacement command. |
...
- The desktop's Tools screen reads `serve.json` (with its process alive) and
  ...
  Setup has its own bounded worker; failure remains on the card, while ordinary
