...
| `pause {from, project?, reason}` | `pause {pause: {project, by, reason, at}}`, `error(invalid\|not_found)` | the person tells a project's agents to hold: one `pause` message from `from` reaches every live agent in the project (archived under `#everyone`, payload `{text: "Pause: <reason>", reason}`), the pause is kept as a document so a restart remembers it, and until it is lifted `claim` from an agent in that project answers `paused` with the reason (what an agent holds, it keeps; the person's own claims are not held). `project` is an id, root or unique prefix, the caller's own when absent; a second pause replaces the reason. Emits `project_paused`. |
| `resume_project {from, project?}` | `ok` | lifts the pause: a `resume` message reaches the project's live agents and their leases are theirs again. `ok` for a project that is not paused. Emits `project_resumed`. |
