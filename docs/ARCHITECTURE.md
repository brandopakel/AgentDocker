...
**Protocol (daemon side in source; the rows in the table above are the contract):** `conversations {project?}` → `[{conversation, kind: everyone|all|channel|collision|dm|notices, name, title, members, unread, last_seq, last_at, last_line}]`; `history {conversation, before_seq?, limit}` → messages with reply counts on roots, newest last, paginated by seq; `thread {message}` → a root and its replies; `mark_read {conversation, through}` → `ok`, event `conversation_read {reader, conversation, through}`; `search_messages {project?, query, before_seq?, limit}`; `channel_open` gains `name?` and `project?`. Schema bump for the archive and the cursors. Existing queues, receipts, bindings and the question lifecycle are untouched; the archive is written beside the inbox rows, not instead of them.
