...
**App.** One *Messages* screen replaces Questions/Inbox and Channels: a sidebar (search, Unread, Threads, Mentions; *Channels* with `#everyone` and the named ones, *Collisions* collapsed, *Direct messages* with the agent's presence from its activity, ended sessions' conversations under a collapsed *Earlier*), a pane with a header (name, topic or task, members), day dividers, the unread divider, sender initials, name, time and the message kind as a small badge, questions inline as cards, hover *Reply in thread*, and a composer that is always there (Enter sends; multiline composition remains open), with a right pane for a thread or the members. The projects rail stays as the Discord server rail, with a three-dot menu per project (remove from the list, rename, reveal in Finder) and a duplicate name shown with its parent folder. Sessions are named by tool and branch (*Claude Code · main*), never numbered, unless two live sessions of one tool share a branch; the *History* tab becomes a collapsed *Earlier* group under the live sessions, kept for exit codes, logs and attribution but out of the way. Identifiers appear only under *Details*; every surface shows the chosen or generated name. Keyboard: a switcher on Cmd+K, the existing accessibility contracts apply.
...
| 40 | ✅ roles: a `role` label an agent is given, `role:<name>` as the recipient of `send`, `ask` and `handoff`, resolved to the one live holder in the sender's project; PR #186 carried by merged #191 | 5 | — |
| 41 | ✅ reply from the notification: a Reply field on macOS message notifications, sent as the person's reply to where the message went, the words kept as the conversation's draft when it did not go; PR #190 carried by merged #191 | 5 | 31 |
| 42 | ✅ per-recipient delivery readiness on `sent` (`recipient_readiness`): which queued sessions cannot be woken and how to relaunch them; PR #188 carried by merged #191 | 5 | 30 |
