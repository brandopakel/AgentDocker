...
Reads are served from memory; every mutation is written through to SQLite (`rusqlite`, bundled, WAL mode) before the response goes out. Rows are JSON blobs of the core types beside the few columns needed for lookups (`agents`, `leases`, `inbox`, `events`, `changes`, `journal` with its `journal_paths` and `journal_fts` indexes and `journal_cursors`; `projects` is the one plain table, a cache of fingerprints per repository root), so adding a field to a core type is not a migration. A `meta.schema_version` row guards against opening a database written by an incompatible build. Schema 11 upgrades schemas 1 through 10 on open and idempotently translates old `file:` leases using each holder's recorded checkout. Schema 8 made a restore point durable launch intent, including an interrupted `created` restore. Schema 9 retains pending question routes across restart. Schema 10 protects retained live deliveries and non-evicting inboxes from older daemons that would drain or discard accepted work. Schema 11 adds durable exact-ID aliases and an explicit offline [identity repair](IDENTITY-REPAIR.md) with atomic operational moves and retained before-images; older daemons must refuse this state. Existing queues above the new byte limit remain intact; admission resumes after acknowledgement frees capacity. Image-bound validation, runner deadlines, container lifetime and process-group tracking remain supported. Compatibility is checked before schema DDL or journal-mode changes. A downgrade requires the matching pre-upgrade state backup, not opening newer state with an older daemon.
