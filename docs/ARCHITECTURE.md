...
| `task_pull {agent, task}` | `task {task}`, `error(conflict\|invalid\|not_found\|ambiguous\|storage_unavailable)` | a running agent takes a Ready card nobody holds: it becomes theirs, in `in_progress`, and the pull takes an exclusive `task:<id>` lease (four hours, the title as its note) in the same commit as the card — the lease is the holding, so `leases` shows who has which card, `renew` extends it, an exit or `release` ends it, and a second claimant is refused like any lease. Under the daemon's state lock, so two agents never get one card: the second is told who holds it (`conflict`, `details.assignee`, `details.column`). A card in `in_progress` or `review` whose holder's lease has lapsed (expired, released, or its agent exited) passes to the next agent that pulls it, where it sits, and the event says `from` whom; a lease on the card held by hand by somebody the card does not name refuses the pull (`conflict`, `details.held_by`). A Backlog or Done card is `conflict`; the person does not pull (`invalid`), they assign with `task_update`. `task` is an id or a unique prefix, looked up as one (two rows read, not the board). Emits `lease_claimed` then `task_pulled {task, project, agent, from?, lease}`. |
| `task_move {agent, task, column}` | `task {task}`, `error(forbidden\|invalid)` | the card's assignee moves it, and the person moves any; a card the person moves back to `backlog` or `ready` is released for the next taker, one its assignee moves to `done` stays theirs for the record. Emits `task_moved`. |
| `task_update {agent, task, title?, acceptance?, assignee?}` | `task {task}`, `error(forbidden\|invalid)` | the person edits any card's words and hands it to an agent (`assignee` an id, name or prefix; `""` takes it away); an agent edits only the words of a card it holds. Emits `task_updated`. |
| `task_archive {agent, task}` | `ok`, `error(forbidden)` | off the board, kept for the record: the person's to do, or the assignee's for a card in `done`. Archiving twice is `ok`. Emits `task_archived`. |
| `tasks {project?, column?, archived?, limit?}` | `tasks {tasks: Task[], more}`, `error(storage_unavailable)` | one page of the board, Backlog to Done and oldest first within a column, at most `limit` cards (1–1,000; 200 by default) with `more` when the board goes on past them — read as a page from the store, so a board of long cards never fills a frame or holds the lock; archived cards only when asked; every project's when none is named. A read: served during a coordinator transfer. |
