...
**What a session leaves on disk, and for how long.** Captured output is bounded: the owner rotates `logs/<id>.log` at 8 MiB, moving what it holds to `logs/<id>.log.1` (replacing the earlier move) and starting over, so one agent keeps at most 16 MiB however long it talks; `logs` reads the earlier half first when everything is asked for or the live file cannot fill the tail asked for, and a follower crossing a rotation reads the rest of the moved file before the fresh one. `rm` deletes both halves with the record. On the minute tick the daemon also removes any log under `logs/` whose agent is not in the registry once it is an hour old, so a record forgotten while the daemon was down leaves nothing behind (validation logs are named for their validation record and stay with it). The desktop app removes its `/tmp/agentdocker-ui-<key>` activation directory, lock file included, when it exits, as the daemon removes its socket directory (see the wire protocol). Retained desktop builds are the remaining large item and are pruned on request (`desktop prune`).
