...
| `task_pull {agent, task}` | `task {task}`, `error(conflict\|invalid\|not_found\|ambiguous)` | a running agent takes a Ready card nobody holds: it becomes theirs, in `in_progress`, in one step under the daemon's state lock, so two agents never get one card — the second is told who holds it (`conflict`, `details.assignee`, `details.column`). A card in any other column is `conflict` too; the person does not pull (`invalid`), they assign with `task_update`. `task` is an id or a unique prefix. Emits `task_pulled`. |
...
| `tasks {project?, column?, archived?}` | `tasks {tasks: Task[]}` | the board, Backlog to Done and oldest first within a column; archived cards only when asked; every project's when none is named |
