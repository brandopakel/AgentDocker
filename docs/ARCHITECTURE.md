...
| `post_question {from, to, question, timeout_secs?}` | `sent` | creates the same durable question and returns its ID immediately; answers use the shared inbox; CLI `ask --no-wait` |
...
| `activity {agent?, project?, all?}` | `activity {activity: AgentActivity[]}` | `unknown`, `starting`, `working`, `blocked {resource, held_by, since}`, `idle`, `finished`; blocked first. Presence or configuration alone never proves idle. |
