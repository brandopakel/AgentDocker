...
| `report_activity {agent, observation: {activity, observed_at}}` | `ok` | explicit `working` or `idle` observation; rejects future/stale reports, ignores older/equal reports, expires after five minutes; commits record and `agent_activity_reported` event together. CLI `report-activity --as`, MCP `report_activity`, and lifecycle hooks share this operation. |
...
- **Derived activity.** The wait queue identifies a blocked agent and its actual resource/holders. Fresh explicit provider observations report working or idle; they expire after five minutes so a lost hook does not assert either state forever. In their absence, daemon contact newer than registration indicates recent coordination for two minutes; otherwise activity is unknown. Registration and adoption alone indicate presence. The `last_seen` column is daemon contact, not a transcript timestamp or proof of model work. CLI, MCP and GUI use the same activity response. See [activity and messaging](ACTIVITY-AND-MESSAGING.md) for adapter coverage and delivery semantics.
