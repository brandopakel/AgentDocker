...
Safe daemon replacement (in git history).
...
The product direction (in git history) defines current delivery priorities. The phases below retain the detailed engineering design; numbered delivery rows are not GitHub PR numbers.
...
desktop distribution (in git history) for commands and limitations.
...
Release automation (in git history) produces installable archives and a
...
turns and preserves drafts. See the existing message audit (in git history)
