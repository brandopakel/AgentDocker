...
Registered, not supervised — that division is the whole design, and it decides everything else about the path. tmux owns the process, so there is no captured log (the output is on tmux's terminal, and `tmux capture-pane` is where it lives), the agent ends when its command ends and the ordinary liveness sweep notices, and `stop` signals the pid tmux reported. What the agent does get is everything that matters: an identity, the project, leases, a read set, a journal cursor, and the pane on its record so `ps` says where to find it. `--in-pane` refuses to combine with `--tty` (two terminals for one agent, and tmux is providing the one) or with `--restore` (a daemon that did not start it cannot bring it back); the CLI refuses both at the argument level and the daemon refuses them again, because MCP, an `Agentfile.toml` and the protocol all bypass the CLI.
