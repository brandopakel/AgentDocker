...
**Conversations.** Every message belongs to exactly one conversation, derived from its destination: `#everyone` is the project broadcast (`Destination::Project`); a named channel `#<name>` is a `Channel` (which gains a `name`: lowercase `[a-z0-9-]`, at most 40 characters, unique among the project's open channels, chosen at `channel_open` or derived from the task; collision rooms the daemon opens keep no name and are listed under a collapsed *Collisions* category, and a `human` record is no longer admitted to one, since it edits nothing); a direct message is the pair of the person and one agent (`Destination::Agent` either way). A message with `reply_to` is a thread reply under its root; `send --reply-to` is the whole thread protocol. Mentions are `@name` in text and change routing not at all; they are found, not delivered.

**History.** Delivery queues hold only what is not yet consumed, so a workspace needs a bounded archive: a `messages` table appended in the same transaction that publishes a message (id, conversation, from, kind, payload, reply_to, sent_at), retained per conversation to a count and to `[messages] retention` in `agentd.toml` the way the journal is, with the same FTS for search. Questions stay questions (cards with controls) and are also in the archive as messages.

**Read state.** The person has a read cursor per conversation (`read_cursors`: reader, conversation, through message, at). Unread is what the archive holds past the cursor; opening a conversation moves the cursor and acknowledges the person's queued rows in it, so *Clear* per message becomes *read* per conversation. Agents keep acknowledging their queues as today; cursors are for readers, not receivers.

**Protocol (planned):** `conversations {project?}` → `[{conversation, kind: everyone|channel|collision|dm, name, title, members, unread, last_at, last_line}]`; `history {conversation, before?, limit}` → messages with reply counts on roots; `thread {message}` → a root and its replies; `mark_read {conversation, through}` → `ok`, event `conversation_read`; `messages_search {project, query}`; `channel_open` gains `name?`. Schema bump for the archive and the cursors. Existing queues, receipts, bindings and the question lifecycle are untouched; the archive is written beside the inbox rows, not instead of them.

**App.** One *Messages* screen replaces Questions/Inbox and Channels: a sidebar (search, Unread, Threads, Mentions; *Channels* with `#everyone` and the named ones, *Collisions* collapsed, *Direct messages* with the agent's presence from its activity), a pane with a header (name, topic or task, members), day dividers, the unread divider, sender initials, name, time and the message kind as a small badge, questions inline as cards, hover *Reply in thread*, and a composer that is always there (Enter sends, Shift+Enter breaks a line), with a right pane for a thread or the members. The projects rail stays as the Discord server rail. Identifiers appear only under *Details*; every surface shows the chosen or generated name. Keyboard: a switcher on Cmd+K, the existing accessibility contracts apply.
