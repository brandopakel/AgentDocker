...
| `task_move {agent, task, column}` | `task {task}`, `error(forbidden\|invalid\|storage_unavailable)` | an agent moves only a card it holds with a live lease — a lapsed hold is `forbidden` with `details.hold: lapsed`, and the card is pulled again by name first; the person needs no hold. A move that ends the holding — back to `backlog` or `ready`, or to `done` — releases the card's leases in the same commit (`lease_released` follows `task_moved`); `done` keeps the assignee for the record. Otherwise: the card's assignee moves it, and the person moves any; a card the person moves back to `backlog` or `ready` is released for the next taker, one its assignee moves to `done` stays theirs for the record. Emits `task_moved`. |
