...
| `report_adapter {agent, adapter, contact: {process_started_at, observed_at}}` | `ok`, `error(invalid)` | recent `mcp` or `hooks` contact, separate from activity and input delivery. Requires a live matching process generation, a nonfuture observation younger than five minutes and a supported hook runtime. Coalesces observations less than 30 seconds apart per adapter/generation; atomically commits the agent and `adapter_contact_reported` event. The hidden CLI is `report-adapter --as --process-started-at mcp\|hooks`. Adapter diagnostics add at most a 250 ms wait to a throttled request; failure or an older daemon does not refuse the provider operation. |
