...
| 27 | 🔄 snapshot restore (implemented; readiness and persistence defects noted in [PRODUCT-DIRECTION.md](PRODUCT-DIRECTION.md) remain open): `run --restore` brings an agent back under its own id after a daemon restart, with its leases re-taken from a restore point and a `restored` brief naming its checkpoint, what it had read, what changed while it was down, and its journal cursor | 5 | 23 |
