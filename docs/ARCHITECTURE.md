...
This document describes implemented behavior and later design intent. The September 6 audit (AUDIT-2026-09-06.md in git history) records known exceptions and test evidence at a pinned revision; implementation does not imply release availability or completion of hardening.
...
Shared host I/O: project/path discovery, Git/content inspection, process identity, installed-runtime/config inventory, bounded subprocesses, terminal operations, notifications, multiplexer queries and optional container transports. It owns no daemon registry or durable coordination state. Windows IPC keeps protected user/SYSTEM pipe ACLs and same-user checks on both ends. The server identifies the connected client through its pipe token, without opening a client process across logon sessions; it restores the thread before returning or interpreting application input. Clients retain server-process and pipe-ACL validation. [Windows connection boundary](WINDOWS-PORT.md#local-connection-boundary).
...
- **Watcher** — `notify` coverage over every checkout a live agent works in, feeding the ledger and branch refreshes; see [Watching and the ledger](#watching-and-the-ledger).
...
`CreateFileW` override before worker threads and connections. It supplies the
current user's explicit owner and protected user/SYSTEM DACL for new database
and companion files, including recreated WAL, journal and shared-memory files.
Existing state still passes strict owner/write-access validation; the override
does not change an existing owner or ACL. Initialization fails closed if the
...
On startup the daemon first refuses ambiguous duplicate live names before changing any agent or lease. A shared name is not proof of a shared identity, and retiring either record could release a running writer's protection. After this preflight, a managed record still `created` without durable restore intent is recorded as failed, and a lease whose holder is not live is dropped — each written back so the store and the registry agree. Reloaded leases keep their original expiry. Opt-in snapshot restoration may acquire new leases from a recent restore point with new IDs and expiries; that is distinct from extending an existing lease. Legacy identity reconciliation uses the explicit offline repair command; durable aliases are validated before those recovery changes.
...
substituted. Complete pruning retains the sequence high-water mark, but a new
checkpoint is unavailable until an anchor event exists again. Cursors are
continuity checks, not authentication credentials or tamper-resistant backups.

`events_ready_at` establishes the validated start, while `events_caught_up`
separately proves replay completion. The CLI's `events --resumable` emits these
JSON frames; `--after-cursor '<complete cursor JSON>'` resumes explicitly after
processed work. The checked client validates identity, consecutive sequences,
event/cursor agreement and the completion boundary. It rejects unknown event
kinds, EOF, malformed/partial/oversized frames, five-second connection/replay
stalls and unfinished frames; an idle live stream has no inactivity timeout.
Consumers retain a cursor only after processing its frame. The stream omits the
explicit live-only `file_changed`/`agent_stale` notifications; their ledger APIs
remain their recovery path. Broadcast lag terminates explicitly and requires
another checked resume. This protocol alone does not reconnect a provider,
preserve its pending work or enable daemon replacement; see
[live daemon replacement](#phase-5--the-machine-and-the-human) (row 28).
...
- **The channel is the parent's choice.** `--claude-channel` serves the Claude channel only when the parent `claude` was started with `--dangerously-load-development-channels server:agentdocker` — read from the parent process's arguments — or with `AGENTDOCKER_CLAUDE_CHANNEL_INPUT=1` from a managed launch. `agentdocker setup --shell` puts the flag on every terminal `claude` through a marked block in the shell's startup file (`agentdocker_host::runtimes::shell`), and `RuntimeInfo.shell` reports whether it is there.
...
**Discovery.** Agents that never register are still worth seeing. `discover` reads the process table once (`ps -axo pid=,ppid=,args=`, portable across macOS and Linux and complete enough to recognise `node …/@anthropic-ai/claude-code/cli.js` as well as a native `claude`), keeps the rows whose command line matches the known-runtime table in `agentdocker_host::procinfo` (`claude-code`, `codex`, `gemini-cli`, `cursor`, `aider`, `goose`, `copilot`, `amp`, `opencode`), drops pids that live agents already claim, and reads each survivor's working directory (`proc_pidinfo` on macOS, `/proc/<pid>/cwd` on Linux) to place it in a project — without a fingerprint, because this runs on every `ps` and a process nobody adopted should neither warm the cache nor announce a repository. `ps` appends them, dimmed on a terminal and plain in a pipe, under the name `adopt` would give them (`<runtime>-<pid>`) with status `unadopted`, and says so on stderr; `--no-discover` skips it. `adopt <pid>` registers the process with the runtime from the table (overridable), the working directory from the process, the pid for liveness, and the label `adopted=true`. A known runtime's *helper* is not a session and never becomes one: Claude Code's `--chrome-native-host` (the bridge the browser launches through native messaging so its extension can reach Claude Code), its `daemon`, its `bg-spare` (the process that holds a session's model and registers itself through its own hooks and MCP server once a terminal takes it), the `attach` terminal in front of such a session (retitled `agents` as its screen changes; it is that session's screen, not a second session), Codex's `app-server` sidecar. `procinfo::helper_of` names them; discovery does not list them, and `adopt` refuses them by pid with what they are (`invalid`) whichever runtime the caller names, because registered they would appear as an agent nobody can reach in a project named after the helper's directory — which reads as "my browser agent is connected" when it is not. Adoption alone installs no hooks and supplies no activity evidence or automatic inbox consumption. The agent is visible and counted in its project; messages wait until an adapter consumes them. A subsequently connected adapter can reuse the record when its process, birth time, runtime, physical checkout and provider session match. It is a heuristic on-ramp and is presented as one.
...
**Reading it.** `journal {project, since_seq?, until_seq?, agent?, branch?, kind?, path?, grep?, limit?}` returns the newest `limit` entries oldest first with the resolved project id; `agentdocker journal [--project .] [--since N] [--until N] [--agent A] [--branch B] [--kind K] [--path P] [--grep TEXT] [-n N] [--follow]` prints them, `journal add "…"` appends a note, `journal prune --before <seq|duration>` trims. `release --summary "…"` (and the MCP `release` tool's `summary`) is how an agent says what it did; the MCP `journal_note` tool is how it leaves a note. Every append is announced as `journal_appended`, which `--follow` streams (the stream is opened before the snapshot it continues, so nothing falls between them, and the listing's filters apply to streamed entries too).
...
|---|---|---|
| `worktree_create {agent, path, branch, from?}` | `worktree {path, branch}` | host-only; new linked checkout on a new branch, at the agent's HEAD or at `from` (a branch, tag or commit; an argument-shaped `from` is refused). The daemon remembers the agent that asked, so a commit made there with git alone is journaled as that agent's while it lives |
...
| `resume_session {agent, spec}` | `agent`, `error(invalid\|conflict\|not_found\|ambiguous\|storage_unavailable\|unavailable\|internal)` | bring an ended session back under its own record: `spec` is checked against the record before anything starts (`identity::relaunch_check` — the record is not live, local, outside a container, has no input binding, a plain `session_id` label of 1–128 `[A-Za-z0-9_-]` starting alphanumeric, runtime `claude-code` (Codex resumes as `codex resume <id>`; that route is not built) and the same in `spec`; `spec` is a plain process, not paned, isolated or restored, in the record's own `workdir`, and its command carries `--resume <session_id>`), a subscriber on the record answers `conflict`, and so does the old process when its pid is still alive with the recorded start time or alive and undatable (`same_process`, fail closed). The record becomes managed with the launch as its spec (its name and labels kept, `session_id` included), its process fields cleared, and the transition is committed with `session_relaunched {agent, session}` under the fence before memory changes — every check is run again under that lock, and a record that differs in any field from the one checked (it went live, ended again, took a role or a report meanwhile) answers `conflict` rather than being written over; a write that fails leaves the ended record as it was (`internal` is the checkout resolution not completing, before any of this). Then the process starts as `run` starts one, under the same id, so what was queued for the session, its aliases, questions and cards are its own; a launch that fails or exits at once leaves the record ended again with its queue, and never a second live process. CLI `reconnect`; the app's **Reconnect here**. |
...
| `register {spec, pid?, session?}` | `agent` | external process; PID must be positive and fit i32; `spec.workdir` decides the project; `session` is the multiplexer the client can see it is in, read first-hand from its own environment. A registration that names a provider session (a non-empty `session_id` label, which only a hooks adapter vouches for) whose ended record is still held — same runtime, project and physical checkout, its process gone, neither side managed, contained, in a pane or bound to an input controller — is that session come back: the ended record that finished last stays canonical and takes the new pid, birth, status and last_seen, keeps its id, conversations, cursor and what was still queued for it; the fresh record and every earlier ended record of the session are retired into it — their queues one queue in durable sequence order with each message once (a broadcast that reached two lives), every document that named them rewritten the way the offline identity repair rewrites it (a question an earlier life asked is the canonical record's to cancel; open and closed channels keep their membership, opener and reviewer references under the canonical ID; duplicate members collapse to one; an access grant, a restore point or a rewrite that would make a self-review or self-question refuses), an alias that pointed at a retired record pointed at the canonical one, their cursors gone and their ids `identity_alias` documents — planned and written by the store as one transaction with the event (`plan_resume`/`write_resume`), announced by `session_resumed` naming every retired id. A record whose process still runs, or one that holds leases, waits on a lease, has stale changes owed to it, has a live subscriber, or a fresh record that already has input-delivery evidence (`fresh.input_delivery.is_some()`), is not folded (the fresh record then keeps its own); a duplicate message that differs in content, a store that cannot say or a refused rewrite leaves everything as it was. |
...
| `runtimes` | `runtimes {runtimes: RuntimeInfo[]}` | the agent tools on this machine — CLI and version, desktop apps, config directory, whether the MCP server and hooks are wired in, and when the hooks are missing which required events lack our command (`hooks_missing`; every event when there is no file or hooks are disabled wholesale, none when the file is unreadable, malformed or structurally invalid; those files are Unverified, including a non-Boolean `disableAllHooks`), for Claude Code whether the shell's startup file carries the channel flag (`shell`: Wired, Missing, Unverified, or Unsupported for a shell setup does not know) — with the unregistered running processes of each, and the vendor's browser extension per Chromium-family browser profile it is installed in (`extensions`: label, browser, profile when the browser has several, version, and the native messaging bridge registered for it, found by the extension's id among a manifest's `allowed_origins`). A runtime that has only an extension (`claude-browser`, `chatgpt-browser`) works inside the browser: its sessions are never listed, and every surface that shows the row says so (`runtime::IN_BROWSER`) |
| `adopt {pid, name?, runtime?}` | `agent` | registers such a process; `invalid` if a live agent already has the pid, or if the pid is a known runtime's helper rather than a session (the message says which helper) |
| `stop {agent, force?}` | `agent` | validated SIGTERM/SIGKILL; returns `stopping` until observed exit, retaining leases |
| `remove {agent}` | `ok` | forget a finished agent; `invalid` while it is live, saying `stop` for a managed one and `deregister --as` for a process AgentDocker did not start, so that forgetting an adopted record never signals its process |
| `list {all?, project?, labels?}` | `agents` plus optional `aliases` (former ID → canonical ID) for returned records | live only unless `all`; `project` is an id prefix or an absolute path inside it; `labels` must all match |
| `inspect {agent}` | `agent` | |
...
| `checkpoint {agent,key,task,assumptions?,next_steps?,links?,release_leases?}` | `checkpoint` | persist context before optional release; retries are idempotent |
...
| `checkpoint_prune {agent?, older_than_secs}` | `pruned {removed}` | forget checkpoints older than that whose author, acceptor and handoff addressee have all finished, with the handoff bundles carrying them |
| `handoff {agent, to?, task?, note?, links?, transfer_leases?, key?}` | `handoff {bundle}` | a checkpoint addressed to `to` (an agent ref or `role:<name>`) with the sender's state bundled around it, announced to `to` as a `handoff` message; leases are released unless they are to move at acceptance; without `to` the bundle is an export; retries with the same key return the same bundle |
...
| `channel_open {agent, task, members?, name?, project?}` | `channel {channel}`, `error(conflict\|invalid)` | a room for a task; members default to every other live agent in the project. `project` (id or path) is the project the room belongs to when the opener is in none or in another — a person opening a room from the app names the one they are looking at — and the opener's own otherwise; an opener in no project with none named is `invalid`. `name` is the `#name` people use (lowercase letters, digits and hyphens, at most 40, unique among the project's open channels; made from the task when absent, so `Plan the release` opens `#plan-the-release`); the id stays the identity, a closed channel keeps its name in history and reuse makes a new id. A record of runtime `human` is never admitted to a collision room. |
| `channel_invite {agent, channel, member}` | `channel`, `error(forbidden\|invalid\|storage_unavailable)` | an existing member adds one live agent to an open task channel; membership, the `channel_invited {channel, by, agent}` event and invitation notice commit together. A repeated member is a no-op. Closed and collision channels refuse invitations. The new member receives subsequent channel messages through the ordinary queue. |
| `conversations {project?, reader?}` | `conversations {conversations}` | what a reader (the person unless `reader` names an agent) sees: one summary per conversation, `{conversation, kind: everyone\|all\|channel\|collision\|dm\|notices, name?, title, members, unread, mentions?, last_seq?, last_at?, last_from?, last_line?}`, newest activity first. `mentions` (absent when zero) is how many of the unread rows name the reader with `@`: the reader's current name and names saved at retirement in optional alias metadata (`retired_name`), and `@user` or `@you` for the person. Legacy aliases without that metadata cannot reconstruct the former name; `@name` is a name where it stands on its own (`@codex-51242,` counts, `a@b.c` does not), compared without case. Every archived message belongs to one conversation by its destination: `everyone:<project>`, `all`, `channel:<id>`, `dm:<a>:<b>` (the sorted pair of ids, whoever the parties are) or `notices:<agent>` (what `agentd` itself told one agent); topics are streams and are not archived. A project lists its broadcast, its channels, the reader's direct conversations with its agents and anything archived; without a project, every project's plus `all`. A reader sees rooms and broadcasts by membership and direct conversations and notices as a party; the person, in no room, sees every room, direct conversation and notice of the project (to read: a direct conversation between two agents has no destination for anyone else); a reader is every identity it has had, so a record retired into it by an identity repair or a resumed provider session keeps its conversations under their old ids, the furthest cursor any of those identities left counts, and `unread` counts archived messages past that cursor that none of those identities sent. At the archive's backfill (schema 21) the person's cursors start at every conversation's head, so what was queued before there were conversations is not the first unread count. |
| `history {conversation, before_seq?, limit?}` | `history {messages}` | the newest `limit` (default 100, at most 500) archived messages before `before_seq`, oldest first, each as `{seq, conversation, ...envelope, replies?}` where `replies` counts the thread under a root. The archive is written in the same transaction as the inbox rows, beside them, never instead of them; a topic message has no archive row; a broadcast nobody was live to receive is archived (it was said), a send a policy refused is not. Retained per conversation to 5,000 rows and to `[messages] retention` in `agentd.toml` when set, pruned on the minute tick within one budget of 1,000 rows for the whole tick across the age cutoff and every crowded conversation (`messages_pruned`), never a conversation's newest message, so its head and last line survive. Search uses FTS5 while the index answers and falls back to LIKE from the first index error until the next start rebuilds it. At the schema 21 upgrade the archive is backfilled once, by message id, from the rows still queued: nothing acknowledged earlier exists anywhere, so history starts there. |
| `thread {message, after_seq?, limit?}` | `thread {root, replies}`, `error(not_found)` | one archived message and the replies threaded under it: same conversation, oldest first, after `after_seq`, at most `limit` (default 100, at most 500); page by the last reply's seq. A reply whose `reply_to` names a message in another conversation is a plain message that keeps the id as data. |
| `mark_read {conversation, through, reader?}` | `ok`, `error(invalid)` | the reader read the conversation through an archived `seq` of it (`invalid` otherwise): the cursor moves forward, never back (a lower `through` is a no-op), later arrivals stay unread, and only the reader's own queued rows that the cursor covers are acknowledged, in the same transaction as the cursor and its `conversation_read {reader, conversation, through}` event. A reader whose queue has an owner (a bound controller, the daemon's own bridge) moves its cursor and drains nothing: the owner's receipts do that. Reading never closes a question: a question closes through an accepted answer, its asker's cancellation or its expiry; other agents' queues are untouched. |
| `search_messages {query, project?, reader?, before_seq?, limit?}` | `history {messages}` | archived messages whose text matches (FTS5, or LIKE without it), newest first, paginated by `before_seq`, among the conversations the reader (the person unless `reader` names an agent) could list with `conversations` for that project or everywhere: an agent searches only what it is in. |
...
| `peek_input {agent}` | `messages` | look at a queue without consuming it or counting as a delivery, whoever consumes it: for a person or the app, never wired into a session's own reader; records no offer |
| `provider_inbox {agent, acknowledge?: MessageId[], token?}` | `messages`, `input_batch {agent, messages, uncertain, answers_routed}` | the exclusive consumer's read: acknowledge exact provider receipts (never mere queue acceptance) and return the same retained Send queue. For a managed Codex bridge session the answer is `messages`, as before. For an agent with an input binding the `token` is required (`error(forbidden)` without it or with another) and the answer is `input_batch`, whose `uncertain` names the queued messages a legacy reader had been offered before the binding, so the controller reconciles them against the provider before enqueueing anything. During a provider block, return an empty offer without draining retained rows. Legacy consumers of either kind of owned queue are answered `input_owned`. |
| `bind_input {agent, provider: {process: {pid, started_at}, session, profile}, controller: {pid, started_at}, token, launch?: {executable, args, cwd, env}}` | `input_bound {agent, binding, resumed}`, `error(invalid\|forbidden\|conflict)` | make one external controller process the sole consumer of the agent's queued input, bound to one exact provider generation. The provider process must be the agent's registered pid and birth, the session its registered `session_id` label, the profile an absolute path, and both processes running; a managed Codex bridge session cannot be bound. The token is the controller's own (32 to 128 printable characters), made and kept before the first bind; the daemon stores only its SHA-256, which is visible on the agent record and in `input_bound`. The token tells one cooperating process of the same OS user from another, so a lost reply and a restart are recoverable; it is not a security boundary: whoever can open the socket already owns the state directory that holds the digest, so nothing is hidden from them that the record does not hold anyway. The same provider, controller and token bind again as a no-op, so a lost reply is recoverable; a different controller with the token resumes the binding once the bound controller has ended (`resumed: true`, `controller_generations` counts up, the queue is untouched) and is refused with `conflict` while it still runs; another token is `forbidden`; another provider generation is `conflict` even after that provider ended, until an explicit unbind. Queued messages a legacy reader had been offered become the binding's `uncertain` set. One thread of one profile has one queue: a bind for a session and profile another record is already bound to is `conflict` (two receivers would feed one persisted conversation); the route for a dead predecessor is `resume_input`. An optional `launch` descriptor (absolute `executable` and `cwd`, at most 64 `args` and 64 `env` entries, kept on the agent record in the open, so no secret belongs in it) is how the daemon starts the controller again once it has ended: it is fixed at the first bind, a resume sends the same one or none, and a different one is `invalid` until an unbind. Once a second the daemon checks the bound controller by pid and birth. When it is gone it emits `input_controller_ended`, marks the agent's `input_delivery` paused (a still-fresh `ready` heartbeat from the dead controller must not show delivery verified), and, while the provider process is still running, starts the descriptor in its own process group with stdin closed, stdout and stderr appended to `logs/<agent>.controller.log`, the daemon's environment plus the descriptor's and `AGENTDOCKER_HOME` set: `input_controller_launched` with the attempt number, or `input_controller_launch_failed`. Launches back off 0, 2, 4, 8 and 16 seconds from the last end, at most five per episode, after which `input_restarts_exhausted` is emitted and the binding stands, dead, until a controller binds by itself, a person retries (`retry_controller`) or somebody unbinds; a controller that stayed bound for a minute starts the count over. The started process binds itself with the same token, resuming the binding (the daemon never rebinds, never touches the provider generation, and restarts receivers, never provider sessions); one that has not bound within 30 seconds is sent SIGTERM, then SIGKILL five seconds later. The daemon holds a shared installation pin on the descriptor's executable for the binding's life, so release retention cannot remove the version a dead controller will be started from; a bind or resume whose release cannot be held answers `unavailable` while the release is being removed, `not_found` once it is gone, else `invalid`. The restart record (`binding.restart`: attempts, launched process, last end, exhausted) is on the agent record, so a replaced daemon continues the count. Emits `input_bound`. |
| `upgrade_controller {agent, provider, controller, previous: {executable, args, cwd, env}, launch: {executable, args, cwd, env}, token}` | `input_bound {agent, binding, resumed: false}`, `error(invalid\|forbidden\|conflict\|transferring\|storage_unavailable)` | schema22: authenticate the existing binding and compare the exact provider/controller/previous launch. Only the executable may change. Retain the new release pin and commit replacement intent before stopping the old receiver through normal supervision; a fence or failed write leaves it running. Pending inputs, token and receipts remain in the same ledger. A repeated accepted descriptor is a no-op, including after a lost reply; success accepts replacement intent, while CLI readiness polling proves a new receiver is running. |
| `resume_input {agent, predecessor, provider: {process, session, profile}, launch}` | `input_resumed {agent, retired, binding}`, `error(invalid\|conflict\|not_found)` | a provider session that came back as a new process registered as a new record (registration matches by pid and birth), while its thread's queue, binding and controller ledger sit on the record of the process that ended. The caller is the new record, live and unbound, and `provider` must be its own registered pid and birth and its `session_id` label; `predecessor` names the prior record, which must be of the same runtime, bound to the same thread and profile with the same canonical checkout, must be the only record so bound, and whose provider process, bound controller and any launched replacement must all be gone; a caller that already holds leases, sits in a channel or has recorded observations is refused rather than merged. One transaction: the prior record stays canonical and takes the caller's pid, birth, status and last_seen; its binding takes the new generation and this launch descriptor with the restart episode reset so the next tick launches; the token digest stays, what was `uncertain` stays and the caller's own legacy offers join it, nothing else is flagged (the controller's ledger proves what it never prepared); the caller's queued messages move across in `sent_at` order; the caller's id becomes an `identity_alias` of the prior id, the same document the offline repair writes, so every later request naming it resolves to the prior record. Emits `input_resumed`. A lost reply asked again arrives through the alias: the same predecessor, generation and descriptor get the same `input_resumed` with nothing recorded; anything else is `conflict`. |
| `retry_controller {agent}` | `ok`, `error(not_found\|invalid\|conflict)` | a person's retry after the daemon gave up starting a bound controller (`agentdocker provider retry <agent>`; the desktop app's **Restart message delivery** on the selected session, shown once the binding's restarts are exhausted): the restart episode starts over on the binding as it is, so the next tick launches the descriptor again; the queue, the token and the provider generation are untouched. `invalid` for a binding without a launch descriptor (its controller is started by the runtime's own hook), `conflict` while the bound controller or a launched one is running. Emits `input_restarts_reset`. |
...
| `claim {agent, resource, mode?, amount?, ttl_secs?, note?, wait_secs?, automatic?}` | `lease`, `error(conflict)`, `error(deadlock)`, `error(forbidden)` or `error(paused)` | a `conflict` says by whom, by name: `<resource> is held by <name> (<id>; <mode> on <resource>)`, with the leases in `details.held_by`; `amount` spends a `quota:` resource and is ignored for every other kind; policy is checked once, before the first attempt | `path:` uses canonical physical absolute keys; `file:` is a validated checkout alias; conflict `details.held_by` lists the blocking leases; `wait_secs` (max 600) queues in arrival order and retries when it is this waiter's turn; a wait that would close a cycle is refused at once with `details.cycle`; `automatic` marks a lease an adapter took for the edit in hand rather than one the agent asked for — the hooks adapter's per-file lease — and a holder's re-claim of its own lease renews it, deliberate if either claim was |
| `activity {agent?, project?, all?}` | `activity {activity: AgentActivity[]}` | `unknown`, `starting`, `working`, `blocked {resource, held_by, since}`, `idle`, `finished`; blocked first. Presence or configuration alone never proves idle. `queued_inputs` is the durable inbox count and `awaiting_receipt` how many of those no receipt from the current process names (both absent on older daemons). |
| `role {agent, role?}` | `agent`, `error(invalid)` | give a live agent a role, or take it away with no `role`: one word of lowercase letters, digits and hyphens (at most forty), kept as the `role` label on the record and committed with a `role_set {agent, project?, role?}` event; the same role again changes nothing; a finished agent has none. CLI `role`. |
| `rename {agent, name}` | `agent`, `error(invalid\|name_taken\|not_found)` | give a live agent the name a person chose: one to sixty-four characters with no control characters or surrounding space, and not `project`, `all`, `user` or anything starting `project:`, `topic:` or `role:` (those address something else); unique among live agents (`name_taken`); the generated-name label is dropped so the name reads as chosen, the id and every reference by id are unchanged, and the change is committed with an `agent_renamed {agent, project?, name}` event; the same name again changes nothing; a finished agent keeps its name. Refused on the restricted endpoint. CLI `rename`. |
...
| `report_adapter {agent, adapter, contact: {process_started_at, observed_at}}` | `ok`, `error(invalid)` | recent `mcp` or `hooks` contact, separate from activity and input delivery. Requires a live matching process generation, a nonfuture observation younger than five minutes and a supported hook runtime. Coalesces observations less than 30 seconds apart per adapter/generation; atomically commits the agent and `adapter_contact_reported` event. The hidden CLI is `report-adapter --as AGENT --process-started-at RFC3339_TIMESTAMP mcp\|hooks`. Adapter diagnostics add at most a 250 ms wait to a throttled request; failure or an older daemon does not refuse the provider operation. |
| `report_input {agent, process_started_at, observed_at, report, token?}` | `ok`, `error(invalid\|forbidden)` | adapter-only ready/paused or received evidence; while an input binding stands the bound controller's `token` is required and its reports are accepted for any runtime, so nobody else can report readiness or a receipt for that queue; matches the live process birth and provider, rejects stale/future observations, older reports and conflicting equal-timestamp observations, validates bounded receipt data and records only exact IDs still queued for that agent; repeated/unknown IDs are no-ops, and commits the agent plus `input_delivery_reported` before the adapter's separate ACK. Paused reports retain a nonblank plain reason of at most 2,048 bytes; adapters use the outer error context, not the full provider error chain. Ready and receipts for still-queued input clear the pause and retain receipt history. An already-ACKed receipt is a no-op even if it exactly matches the latest receipt: it cannot prove a later pause has recovered. CLI `report-input --as --process-started-at --json` is hidden from normal help. |
| `report_provider {agent, process_started_at, observed_at, report}` | `ok`, `error(invalid/conflict)` | generation-bound, fresh structured `blocked {issue}` or `recovered {blocked_at}` report. Persists the record and `provider_availability_reported` atomically; identical blocks are coalesced. No receipt or task completion is implied. CLI `provider report AGENT --kind KIND`; MCP `report_provider_status` binds the caller. |
| `resume_provider {agent, blocked_at}` | `ok`, `error(conflict)` | explicit user recovery for the exact observed block; refuses stale views and backward clock changes. Clears only that block, preserving queue IDs, receipts, questions and uncertain-input protection. CLI `provider resume AGENT`; desktop **Resume delivery**. |
| `delivery_queue {agent}` | `messages`, `input_waiting {agent, blocked_by, availability, queued}`, `input_owned {agent, owner, controller?, since?}` | non-destructive adapter read of the shared Send queue, gated by local or explicitly shared provider limits. An owned queue, a managed Codex bridge session's or a bound controller's, answers `input_owned` rather than an error: there is nothing for a legacy reader to deliver. Every legacy offer is remembered per message (first time only, on the agent record) for a controller that binds later, and while a binding stands the offered messages join its `uncertain` set at once. `inbox` (draining or not), `ack_inbox` and `subscribe` on a bound queue answer `input_owned` the same way, except that `ack_inbox` is still accepted for exactly the messages named `uncertain` by the binding, a legacy reader finishing its in-flight delivery. A non-draining read is refused on a bound queue because it may be the session's own reader putting the text in front of the model, which the daemon cannot tell from a person, and an exposure that lands between the controller's snapshot and its enqueue could never be reported to the controller in time; a person or the app looks with `peek_input`. A managed bridge session keeps its non-draining `inbox` read for the app, and any legacy offer recorded while unbound joins a later binding's `uncertain` set. While blocked, `delivery_queue` returns `input_waiting`; legacy `inbox {drain: true}` returns `error(conflict)` and retains all messages. Administrative non-draining `inbox` reads remain available. The exclusive `provider_inbox` path acknowledges proven receipts first, then returns an empty `messages` offer while blocked, retaining pending input. A new adapter connected to a pre-schema18 daemon falls back to its legacy non-destructive Inbox only on an explicit unknown-operation response; other errors never bypass gating. Proven receipts can still be acknowledged. |
| `waiting` | `waiting {waiting: Waiter[]}` | the claim queue, oldest first |
| `task_create {from, project?, title, acceptance?, column?, links?}` | `task {task}`, `error(invalid\|not_found)` | a card on the project's board: a title (1–200 characters), what done means (`acceptance`, at most 4000), and a column (`backlog`, `ready`, `in_progress`, `review`, `done`; Backlog when absent). `project` is an id, root or unique prefix, the caller's own when absent. A card is a `task` document, its id twelve hex characters; `task` is `{id, project, title, acceptance, column, assignee?, created_by, created_at, updated_at, archived_at?}`. Emits `task_created`. |
| `task_pull {agent, task, take_over_from?}` | `task {task}`, `error(conflict\|paused\|invalid\|not_found\|ambiguous\|storage_unavailable)` | a running agent takes a Ready card nobody holds: it becomes theirs, in `in_progress`, and the pull takes an exclusive `task:<id>` lease (four hours, the title as its note) in the same commit as the card — the lease is the holding, so `leases` shows who has which card, `renew` extends it, an exit or `release` ends it, and a second claimant is refused like any lease. Under the daemon's state lock, so two agents never get one card: the second is told who holds it (`conflict`, `details.assignee`, `details.column`, `details.hold: live`). A pull of one's own held card renews the lease (a retry after an uncertain reply changes nothing). A card in `in_progress` or `review` whose holder's lease has lapsed (expired, released, or its agent exited) is **not** taken by a plain pull: `conflict` with `details.hold: lapsed` and the holder to name. Recovery is explicit: `take_over_from` names the holder the caller expects to take it from (itself, re-taking its own lapsed hold); the card passes where it sits and the event says `from` whom, and the pull is refused (`conflict`) when the card names somebody else, sits in another column, or that hold is live after all. A lease on the card held by hand by somebody the card does not name refuses any pull (`conflict`, `details.held_by`). A Backlog or Done card is `conflict`; the person does not pull (`invalid`), they assign with `task_update`; a paused project refuses the pull like any lease (`paused`). `task` is an id or a unique prefix, looked up as one (two rows read, not the board). Emits `lease_claimed` (or `lease_renewed`) then `task_pulled {task, project, agent, from?, lease}`. |
| `task_move {agent, task, column}` | `task {task}`, `error(forbidden\|invalid\|storage_unavailable)` | an agent moves only a card it holds with a live *exclusive* lease (a shared claim on the card, made by hand, is no hold) — a lapsed hold is `forbidden` with `details.hold: lapsed`, and the card is pulled again by name first; the person needs no hold. A move that ends the holding — back to `backlog` or `ready`, or to `done` — releases the card's leases in the same commit (`lease_released` follows `task_moved`); `done` keeps the assignee for the record. Otherwise: the card's assignee moves it, and the person moves any; a card the person moves back to `backlog` or `ready` is released for the next taker, one its assignee moves to `done` stays theirs for the record. Emits `task_moved`. |
...
| `release_all {agent, summary?, summary_source?, only_automatic?}` | `leases` | every lease the agent holds, or with `only_automatic` just the ones an adapter took automatically, so a worktree, branch or build-campaign lease the agent asked for outlives a turn; the reply lists what was released |
...
| `journal_prune {project, before_seq?, older_than_secs?}` | `pruned` | drops entries below `before_seq`, older than `older_than_secs`, or both (whichever keeps more); one is required |
...
| `resume_events {after?: {log, seq, digest}}` | `events_ready_at {cursor}`, contiguous `event_at {cursor,event}` replay, `events_caught_up {cursor}`, then live `event_at` | retained coordination events only; subscribe and validate history under one state lock; missing/changed/oversized history gives `event_history_lost` before readiness; storage failures give `storage_unavailable` |
...
- **Agent** — one recipient, resolved by id/prefix/name before publishing; `role:<name>` is the one live agent holding that role in the sender's project (anywhere, when the sender has no project or the reference is used outside a message), `not_found` when none does and `ambiguous` when several do.
...
At most eight sinks, each listening to at most 64 kinds. The file and every secret are read off the runtime at start and every five seconds (the secret through a handle that follows no symlink, so what is checked is what is read); a change to the file or to a secret stops every running sink and starts anew under the next generation — nothing queued is carried over, so an event queued under one address and secret can never go to another — and an unreadable file or secret keeps the last good sinks and is said once, without the file's text (a TOML error is given by position, never by snippet). A project-scoped sink posts only events that name that project; an event that names none is not its. Each sink is two tasks off the state lock: an intake that never waits on the receiver and keeps a bounded queue (256 events, 1 MiB; the oldest is dropped and counted), and a sender making one request at a time under a 10-second deadline for connecting and sending together, no redirects, TLS verified against the platform's roots. What is posted is a projection, never the raw record: `{delivery, event, seq, at, text, ids}` — the kind, the sequence, the time, one line built from identifiers, and the identifier fields the kind carries (`agent`, `project`, `by`, `from`, `to`, `resource`, `message`, `task`, `question`, `requester`, `held_by`, `channel`; a lease's holder and resource); no payload, answer, reason, environment or free text of anyone's. A body over 64 KiB is dropped as `too_large`. Headers: `X-AgentDocker-Event` (kind), `X-AgentDocker-Delivery` (one id for a delivery across its retries), `X-AgentDocker-Timestamp` (Unix seconds) and `X-AgentDocker-Signature: sha256=<hex>` of HMAC-SHA256 over `"{timestamp}.{body}"` with the file's secret, so a receiver can refuse both a stranger and a replay. A 2xx is delivered; a 429, a 5xx or a network failure is tried three times in all (after 1 s, then 5 s, or a `Retry-After` of at most 60 s when longer); any other 4xx is dropped at once. What could not be delivered — refused, unreachable, too large, or dropped for room while the receiver stalled — is counted and announced as `webhook_failed {name, kind, reason, dropped}` at most once a minute per sink — a loss inside that minute is said when it lifts, whether or not anything else arrives — even after deliveries succeed again, committed through the ordinary event path, never posted to any sink, with the sink named by its configured name — the address and the receiver's words never appear in logs or events. This is best effort by design: there is no outbox, so a sink that is down longer than its retries, a daemon restart or a handover loses deliveries. The record stays in the event log and the journal; a webhook is a way to hear about the floor, never the record of it.
...
Schema 16 adds `codex_permissions` question presentations: directory, reason,
concrete read/write/deny paths and network access. The complete fallback text and
native controls describe the same turn-scoped grant, limited to 16 paths and
...
**What a session leaves on disk, and for how long.** Captured output is bounded: the owner rotates `logs/<id>.log` at 8 MiB, moving what it holds to `logs/<id>.log.1` (replacing the earlier move) and starting over, so one agent keeps at most 16 MiB however long it talks; `logs` reads the earlier half first when everything is asked for or the live file cannot fill the tail asked for, and a follower holds the file it reads, so a rotation between two of its reads loses nothing: it drains the file it holds, and when the name has moved to another file (told apart by identity, not size) drains the held one once more for what was written just before the rename, then reads the new one from its start; a rotated half it has already read is not shown again; one rotation per read is followed. `rm` deletes both halves, and the controller log, with the record. On the minute tick the daemon also removes any log under `logs/` whose agent is not in the registry once it is an hour old (`<id>.log`, `<id>.log.1`, `<id>.controller.log`), so a record forgotten while the daemon was down leaves nothing behind (validation logs are named for their validation record and stay with it). The daemon's own `agentd.log` and each `logs/<id>.controller.log` are written through inherited append-mode descriptors nobody coordinates, so they are trimmed in place instead: past 4 MiB the file is copied to `<name>.1` and truncated, and a line written in the instant between the copy and the truncation is lost — they are diagnostic logs, bounded, not evidence. The desktop app removes its `/tmp/agentdocker-ui-<key>` activation directory, lock file included, when it exits, as the daemon removes its socket directory (see the wire protocol); a launch that took the instance lock on a file removed under it meanwhile (`Lock::is_at`) takes it again on the file the name has now, three times at most, so two windows never both think they are the one for a daemon. Retained desktop builds are the remaining large item and are pruned on request (`desktop prune`).
...
The app's inbox is a per-agent pile that the person clears; the request is a messaging workspace in the shape people already know from Slack and Discord: a sidebar of channels and direct messages with unread counts, one pane that always has a composer, threads, mentions, search. The daemon's routing stays as it is (agents keep their durable queues and receipts; nothing here changes what an agent is delivered); what is added is a *conversation* model for people to read, on top of the delivery model agents consume.

**Conversations.** Every archived message belongs to exactly one conversation, derived from its destination: `everyone:<project>` is the project broadcast (`Destination::Project`); `all` is `Destination::Broadcast`; `channel:<id>` is a `Channel`, whose id is immutable and which gains a `name` as a label (lowercase `[a-z0-9-]`, at most 40 characters, unique among the project's *open* channels, chosen at `channel_open` or derived from the task; a closed channel keeps its name and id in history and reuse makes a new id; collision rooms the daemon opens keep no name and are listed under a collapsed *Collisions* category, and a record of runtime `human` is no longer admitted to one, since it edits nothing); `dm:<a>:<b>` is the sorted pair of stable agent ids for `Destination::Agent`, whoever the parties are, so the person's conversations with an agent and two agents' with each other are each one conversation; `notices:<agent>` is what the daemon itself (`agentd`) says to one agent, a system conversation shown as *AgentDocker*. Topics are streams, not conversations, and are not archived. A message with `reply_to` is a thread reply under its root only when the root is in the same conversation; otherwise it is a plain message that keeps `reply_to` as data. `send --reply-to` is the whole thread protocol; a reply whose root was pruned shows as such. Mentions are `@name` in text and change routing not at all; they are found, not delivered.

**History.** Delivery queues hold only what is not yet consumed, so a workspace needs a bounded archive: a `messages` table appended in the same transaction that publishes a message (a monotonic `seq`, the message id, conversation, from, kind, payload, `reply_to`, `sent_at`), retained per conversation to a count and to `[messages] retention` in `agentd.toml` the way the journal is, pruning keeping each conversation's head sequence, with FTS over the text for search. A broadcast nobody was live to receive is archived (it was said); a send a policy refused is not (it was never sent). At the schema upgrade the archive is backfilled once, by message id, from the rows still queued; nothing acknowledged before that exists anywhere, so history starts there. Questions stay questions (cards with controls) and are also in the archive as messages.

Message archive review follow-up: project-scoped search includes archived direct conversations and notices involving finished records; selected-project conversation lists apply project membership independently of reader membership. An unindexed archive insertion or deletion invalidates the durable FTS completeness marker in the same transaction, including after a prior rollback restored that marker while leaving the index disabled in memory. Equal archive/index row counts alone cannot prove completeness. The [regression evidence](verification/INDEX.md) retains all three pre-fix failures and the corrected focused run.
...
**Protocol (daemon side in source; the rows in the table above are the contract):** `conversations {project?, reader?}` → `[{conversation, kind: everyone|all|channel|collision|dm|notices, name, title, members, unread, mentions?, last_seq, last_at, last_line}]`; `history {conversation, before_seq?, limit}` → messages with reply counts on roots, newest last, paginated by seq; `thread {message}` → a root and its replies; `mark_read {conversation, through}` → `ok`, event `conversation_read {reader, conversation, through}`; `search_messages {project?, query, before_seq?, limit}`; `channel_open` gains `name?` and `project?`, and `channel_invite` adds a live member. Schema bump for the archive and the cursors. Existing queues, receipts, bindings and the question lifecycle are untouched; the archive is written beside the inbox rows, not instead of them.
...
**App.** A project opens its shared *Chat* with a compact list of current agents and visible project and agent terminal actions. *Agents* provides session details; *More* holds Board, History and technical controls. The cross-project *Messages* screen retains search, unread filters, threads, mentions and named channels. Canonical identities have one current direct-message row; older alias conversation keys remain reachable under *Earlier* with their own history, unread state and drafts. System notices and contested-path rooms are folded under *From AgentDocker*. Temporary projects and earlier sessions are folded and paged rather than filling the primary view. Custom agent names are preserved; generated names use the tool, adding a stable short identity when that project contains multiple sessions of the same tool. Branches are context, not identity. The composer stays visible while delivery details scroll beneath it; Enter sends. Project changes preserve conversation drafts and retarget the shared chat. A managed terminal attaches inside the app; an external macOS Terminal action verifies the process and focuses its existing tab. The project terminal opens a shell in the project folder. These presentation changes do not delete registry records, combine distinct concurrent sessions or change queue delivery semantics.
...
- **Notices.** Changes warn readers of the same checkout: an `agent_stale` event per change at once, and one inbox message of kind `stale` per reader on the next second's tick naming every path that changed after its observation since the last notice (`paths`, up to 200, `count`, and the last `changes` per path, up to 50), so a branch switch that touches hundreds of paths is one message, and created, removed and modified for one path within it are one path. While a reader still has its last notice queued nothing more is sent to it; the paths wait and the notice sent once that one has been acknowledged names them all (a queued envelope is never changed), so a reader that has not consumed a notice never accumulates a backlog of them. Up to 10,000 distinct paths wait per reader, and `count` is the number tracked, not a total beyond that cap; a notice is kept under 64 KiB by naming fewer paths and changes, down to the count alone. A notice that still cannot be queued is dropped: `stale` reads content and hooks deny a stale edit whether or not a notice arrived. Hooks surface queued notices and deny stale edits until the affected content is observed and reread. Live ledger events carry their own ledger sequence; replayed daemon events use the ordered event sequence.
...
**Retention.** Entries are kept forever by default: the journal is the audit trail, a busy five-agent team writes roughly a megabyte a day at worst, and SQLite is comfortable at millions of rows. `agentdocker journal prune --before <duration|seq> [--project]` deletes on demand, and an optional `[journal] retention = "180d"` in `agentd.toml` (the daemon's own configuration file in its home, distinct from `policy.toml`) is applied by the once-a-minute tick: per project, the oldest entry is probed first so a project with nothing expired costs one index lookup, then at most 1,000 entries go per tick, `journal`, `journal_paths` and `journal_fts` rows together, so a backlog drains over several ticks instead of one long delete under the state lock. Each pass is announced as `journal_pruned` with its reason (`retention`, or `request` for the command). A cursor below the new floor needs no clamping: a digest shows what is after the cursor, and what is gone is gone. An unreadable or invalid file is logged once and ignored rather than read as "keep nothing"; unknown keys are errors. Freed pages are reused by SQLite; `agentdocker daemon vacuum` reclaims disk when someone wants it back. It holds the state lock while SQLite rewrites the file, which would stall a provider bridge waiting on the daemon, so it is refused while sessions are live unless `--force` accepts that. Checkpoints have their own `checkpoints prune --older-than <duration>`, which spares any checkpoint whose author, acceptor or handoff addressee is still live and deletes the rest with their announcement in one transaction. No roll-up summaries.
...
`agentdocker-ui` is an Iced native window over the same local socket as the CLI. Its four primary destinations are Projects, Inbox, Connections and Settings. Projects combine daemon discovery and private folder pins, restore the last selection, and retain quiet or unavailable roots. Sessions retain their real project/worktree identity; an attached terminal keeps its agent identity across navigation. Project tabs expose journal activity, channels, coordination and bundled CLI commands. The VT terminal retains the existing parser and bounded transport. Setup, answers, launch/stop and installation dispatch checked worker commands; view construction performs no I/O.

`app/sessions.rs` filters current work, attention and retained history without mutating registry identities. Known Codex Node launchers with a native child are omitted during daemon discovery; overlapping UI discovery snapshots require known PID/birth equality before hiding a registered process. `app/shell.rs` owns state transitions and tasks; `app/view.rs` renders observations. `catalog.rs` atomically saves bounded private preferences with one write in flight. `controls.rs` supplies keyboard-focusable controls and `accessibility.rs` maps their semantics to native AccessKit adapters with physical-pixel bounds. The tiny-skia software renderer replaces egui/eframe and does not compile the default GPU renderer or image codecs. CLI-only installs remain independent of GUI dependencies. See [the design contracts](ICED-DESIGN.md) and [desktop guide](DESKTOP-UX.md).

Desktop notifications for messages addressed to the human come from the daemon through the app's notification entry point, so attempts do not require an open window. OS permission or delivery failure can prevent display; display does not prove someone read it. A ready event subscription restores connectivity even when no new agent event arrives, and reconnect refreshes snapshots including the selected journal. Stream lag forces a reported reconnect. The client resolves the canonical daemon home and validates private fallback socket directories. `AGENTDOCKER_NO_AUTOSTART` disables startup attempts; otherwise startup failures report early child exit and reap only the client's own child. Full Windows daemon and terminal support remains separate platform work.
...
- **Derived activity.** Fresh provider observations distinguish `working` and
`idle`; current blockers and recent daemon coordination may establish waiting
or working activity. Absent or expired evidence yields `unknown`, never inferred
idle. See the Phase 5 activity contract above for precedence and expiry.
...
The UI registers a retained native notification-response delegate before starting the window. Default clicks enqueue navigation; legacy notices open Inbox and dismissal does nothing. `--open-notification JSON` uses the same activation path. One desktop window per home/socket origin is coordinated with a private lock and endpoint (`/tmp/agentdocker-ui-<origin hash>/activate.sock` on Unix, a named pipe on Windows). The queue holds 32 activations, coalesces adjacent identical requests, rejects pressure without evicting accepted items, and validates origin at the receiver. A click for another origin starts the same executable with that child's home/socket environment. The window refreshes snapshots, resolves message/sender IDs and reveals the question/message without submitting drafts; unavailable destinations produce an Inbox fallback. Native posting, actual Notification Center clicks, bundle-registration behavior and signed-release acceptance remain distinct gates ([REMAINING-WORK.md](REMAINING-WORK.md), notification release cases; NOTIFICATION-ROUTING-AUDIT.md in git history).
...
when it is a bounded ASCII identifier, never shell syntax. Guidance is by
runtime: a Claude Code session gets its resume command, a Codex session its
receiver reconnect, and a runtime with no input adapter (`custom`, `gemini-cli`,
`cursor`, …) is told that it has no input receiver and the message waits in its
queue for the agent's own read (`inbox`/`watch --as <id>`, `read_inbox`) —
never an instruction to reconnect a receiver that cannot exist. MCP includes
guidance with each detail. CLI stderr shows the named issues; `ps` has an INPUT column and
...
| 19 | ✅ native desktop app `agentdocker-ui` (Rust, Iced, OS IPC), terminal/console/questions; desktop distribution and onboarding remain incomplete | 5 | 18 |
...
| 30 | ✅ messaging as a workspace: archived conversations, named channels, threads, search, read cursors and unread counts, mentions, new DM/channel from the app, retention (`messages_pruned`), schema 21 | 5 | 21 |
