...
(it relaunches the session in a pane of the app with its conversation and the
channel; accept Claude's prompt there) or use the session-specific resume command
shown in Delivery details or `ps --input-details` from its project folder.
...
- **Reconnect here** in a Claude Code session's Details: once the session has
  exited in its terminal, the app relaunches it in one of its own panes with
  its conversation (`--resume`) and the AgentDocker channel, so it takes
  messages live; Claude's consent prompt appears in that pane. The copyable
  command stays as the alternative.
