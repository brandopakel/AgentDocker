...
- Commits in a private checkout are attributed: a worktree made with
  ...
  lease for its run, keeps one the caller already held, renews it while the
  run lasts, and stops instead of starting on top of another campaign; when
  the daemon cannot tell which session is calling, the run registers itself
  as an agent (`verify-<pid>`, ended with the run) and holds the lease as
  that, so no run is ever unheld because nobody could be named; the scripts
