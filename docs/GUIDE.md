...
| `pause` / `unpause` / `pauses` | `agentdocker pause "sleeping the laptop"` tells every agent in this directory's project to hold: they get the reason as a `pause` message and their new leases are refused until `agentdocker unpause`; `--project` names another project; `pauses` lists what is paused and why |
