...
| Implementation under validation | Legacy duplicate registry reconciliation | [Offline repair](IDENTITY-REPAIR.md) now previews an exact plan and atomically reconciles proven external local Claude/Codex records, preserving inboxes, leases, membership, history and old-ID routes. Local validation passed 752 Rust tests, 54 Python checks and six actual CLI/restart steps. Packaged native checks, CI and final-head review remain. Ambiguous identities and live/managed transfers remain refused. | [Activity and messaging](ACTIVITY-AND-MESSAGING.md), [delivery checkpoints](DELIVERY-PLAN.md), [identity acceptance](INTEGRATION-ACCEPTANCE.md) |
