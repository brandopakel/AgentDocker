...
| Acceptance | Notification release cases | The installed `f5e298f4` preview includes bounded archived-message reveal, fresh-history checks, explicit queue-refusal errors and cancellation when a newer notification arrives; 171 UI tests and 429 native steps/26 checks passed. Earlier installed foreground/background clicks, pending questions, stale destinations and draft preservation passed within their recorded scope. Finish actual notification clicks on this revision, zero-process launch, broader historical/project routes and physical usability on the signed final package. PR #175 final CI/merge remains pending. [Notification audit](NOTIFICATION-ROUTING-AUDIT.md). |
