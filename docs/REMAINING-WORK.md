...
September 15 installed checkpoint: app and production daemon now serve merged
`3c8c2e1` (schema18), including stale-notice coalescing. All four live external
...
| --- | --- | --- |
| Top | Install and verify native input for existing Codex terminals | The native receiver connects the ordinary human/peer queue to Codex 0.154.0's native input queue. Combined `7133023` passed the 962-Rust/70-Python gate and six release-TUI trials including canonical process resume, disconnected questions, generic legacy replies, rate limits and ambiguous-input recovery. The later `2a7656c` gate passed 964 Rust/70 Python checks. Actual trusted lifecycle hooks bootstrap after a first turn, but Codex 0.154.0 emits no SessionStart on a no-prompt fresh/reopened terminal; that binding gap remains reproduced. Schema-20 historical-answer migration passed the release-binary fixture without a duplicate turn. Finish the startup gap, independent review/CI and installed-session checks before closing this row. Other providers still need their own adapters and acceptance; a shared daemon contract is insufficient. See [Codex input](CODEX-INPUT.md) and [source-specific evidence](verification/2026-09-15-native-codex-queue.json). |
...
| Acceptance | Sustained use and unresolved failures | Real coordinated use on September 15 showed one branch switch producing hundreds of stale and contested-channel notices, one per change per path; the daemon now sends one stale notice per reader and one widening notice per channel on each second's tick, and holds further stale paths while a reader's last notice is still queued (see [architecture](ARCHITECTURE.md#projects)). Complete overnight, sleep/reboot, broader queue/retention and checkout workloads. A [one-hour 100-agent/10,000-file daemon trial](verification/2026-09-11-hour-sustained-use.json) passed 1,392,836 cycles, and a [7.5-hour overnight run](verification/2026-09-14-overnight-sustained-use.json) of 1, 10 and 100 agents (2.5 hours each, 10,000 files) passed 35,050 / 350,118 / 3,470,242 cycles with the daemon under 33 MiB and, over the last 100,000 requests of each population, p99 request latency under 10 ms on a host also running provider sessions and build gates; that binary predates session owners and provider availability, and neither run covers sleep/wake or reboot. Diagnose the retained incomplete Iced capture and historical socket timeout: later passing trials do not explain them. See [testing standard](TESTING-AND-BENCHMARKS.md) and [local trial](LOCAL-TRIAL.md). |
