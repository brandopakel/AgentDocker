...
| Implemented; final validation/review pending | Concurrent provider configuration mutation | Canonical-target locks now coordinate guided apply/undo, legacy setup and hook installation across AgentDocker homes; regression checks cover aliases, independent profiles and refusal before writes. Complete the final native/PR gate. Independent provider CLIs/editors remain outside these advisory locks; exact-entry checks and receipts still guard ownership. | [Guided setup](GUIDED-SETUP.md), [delivery checkpoints](DELIVERY-PLAN.md) |
