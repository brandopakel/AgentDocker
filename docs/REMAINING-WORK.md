...
Current source audit: **September 21, 2026 UTC**, main `7a3e0c2`. Installed-component observations below retain their own dates. This is the
...
| --- | --- | --- | --- |
| 1 | Integrate reviewed implementation | Done: #191 `abec6ec`, #196 `bfd79ff`, #194 `615ae79`, #199 `7022ead`, #198 `181d78b`, #201 `57a9d4e`, #202 `14f1c519`, #203 `16bf69a`, #200 `b8314693`, #207 `78f5f9d4`, #206 `eadae709`, #204 `7a3e0c2`; connector changes are also on main. Open: #208 ordinary setup receipts and the multiline composer follow-up. | Finish final-head CI/review and merge the open changes. Peer reviews and source-specific tests remain recorded; a rate-limited review status alone is not substantive review. |
...
| --- | --- | --- |
| Main (September 21) | `7a3e0c2` | Includes shared chat/project terminals (#200), Usage refresh (#207), Windows daemon/CLI foundations (#206) and build-campaign ownership (#204). Open changes remain listed above. |
...
| Top / platform | Full native Windows product | **Daemon/CLI foundation complete in merged #206 (`eadae709`):** native named-pipe startup/status, registry, direct/project messaging, leases and identity-checked stop passed 17 steps on Windows Server 2025. Final-head CI and peer review passed; the [existing record](verification/2026-09-19-windows-slice-one.json) retains earlier stack, ACL and inherited-handle failures. **Still open:** ConPTY/session ownership and attach (implementation underway), the native Codex queue, provider inventory/setup, service startup, installer/update/rollback, broader daemon/CLI suites, graphical and real-provider acceptance. The recorded `OWNER RIGHTS` ACL compatibility case remains open. There is no coworker Windows installer yet. [Windows port](WINDOWS-PORT.md). |
