...
| --- | --- | --- |
| Top | Install and verify native input for existing Codex terminals | The native receiver connects the ordinary human/peer queue to Codex 0.154.0's native input queue. Combined `8b2afe3` passed the 954-Rust/70-Python gate and actual-TUI receiver crash, legacy-answer, rate-limit and ambiguous-input trials. The subsequent same-thread process resume fix preserves the canonical agent and delivery history; authoritative answer routing covers posted and disconnected questions. Finish those combined fixtures, independent review/CI and installed-session acceptance before closing this row. Other providers still need their own adapters and acceptance; a shared daemon contract is insufficient. See [Codex input](CODEX-INPUT.md) and [source-specific evidence](verification/2026-09-15-native-codex-queue.json). |
