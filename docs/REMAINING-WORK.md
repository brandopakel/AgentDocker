...
Current source audit: **September 21, 2026 UTC**, main `78f5f9d4`. Installed-component observations below retain their own dates. This is the
...
| --- | --- | --- | --- |
| 1 | Integrate reviewed implementation | Done: #191 `abec6ec`, #196 `bfd79ff`, #194 `615ae79`, #199 `7022ead`, #198 `181d78b`, #201 `57a9d4e`, #202 `14f1c519`, #203 `16bf69a`, #200 `b8314693`, #207 `78f5f9d4`; connector changes are also on main. Open: #204 build-campaign ownership, #206 Windows daemon/CLI, and #208 ordinary setup receipts. | Finish final-head CI/review and merge the open changes. Peer reviews and source-specific tests remain recorded; a rate-limited review status alone is not substantive review. |
...
| 7 | Coworkers on all three platforms | **Required by the user for the first rollout:** macOS, Linux and native Windows. macOS has local evidence; Linux has graphical/package CI; Windows has core/host foundations only. | Finish the native Windows daemon/client, supervision/ConPTY, provider setup, service and installer/update path. Produce each platform's candidate and run the same first-run, real-provider queue/idle/busy/limit, terminal, draft, sleep/restart and update/rollback cases on declared OS versions/architectures. macOS/Linux availability does not close Windows. |
...
| --- | --- | --- |
| Main (September 21) | `78f5f9d4` | Includes shared chat/project terminals (#200) and Usage refresh (#207), alongside the earlier integrations. Open changes remain listed above. |
...
| Top / platform | Full native Windows product | Named-pipe foundations exist. Finish daemon/client integration, supervision, ConPTY, identity-safe stop/recovery, provider/desktop inventory, user service/session behavior, installer/update/rollback and actual native graphical/provider acceptance. [Windows port](WINDOWS-PORT.md). |
...
| --- | --- | --- |
| Delivered with acceptance remaining | Messaging as a workspace (Slack/Discord shape) | Conversation archive, named rooms, creation/invites, mentions, threads, search and durable drafts are merged. Naming #201 and session presentation #203 are merged; #200 shared-chat integration is merged (`b8314693`) and installed as an earlier local preview. Multiline/Shift-Enter composition is now implemented in source; finish installed visual/physical-input acceptance. Historical aliases keep their own history; separate concurrent sessions are not merged merely because they use the same provider. |
