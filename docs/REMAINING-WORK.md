...
| --- | --- | --- |
| Top | Limits and interrupted-session recovery across all providers/models | The user's September 14 Claude limit is one case in a requirement covering every supported provider, company, model and runtime, including multi-provider and custom/local integrations. Implement explicit provider availability separate from input readiness, with adapter-specific evidence and bounded recovery. Cover session/usage, rate, credit/billing, concurrency and context limits and their reported scope; coordinate confirmed shared limits without pausing unrelated agents. Preserve the shared human/peer FIFO queue, distinguish queued/received/completed work, reconcile uncertain turns without replay, retain drafts/questions, and prevent ping/retry storms or automatic provider/model/account switches. Show only supported limit/reset evidence; a live process or heartbeat is insufficient. Test before/after receipt, mid-tool and pending-answer limits, queue pressure, provider/daemon restart and recovery under the original or explicitly rebound identity. Track coverage and unsupported signals per adapter/provider in the existing audit; Claude-only evidence cannot close this item. Existing leases and approval expiry remain enforced. Implementation and acceptance remain open. See the [delivery requirement](DELIVERY-PLAN.md#provider-session-limits-and-interrupted-work-september-14) and [message acceptance cases](MESSAGE-DELIVERY-AUDIT.md#provider-limit-and-session-exhaustion-acceptance-september-14). |
| Top | Complete provider input and review handling | Local command review now includes concrete additional permissions and connection context, rejects remote/incomplete requests, and checks the offered one-time decisions; bounded actual Allow/Deny conversations and native review controls pass at `8ff5665`; integration remains pending. Finish network-only/stdin review, broader permission forms, MCP elicitation and secret-input presentation; test further actual-provider interruptions, uncertain writes and sustained conversations. Preserve one queue/order and exact receipts for human and peer input. Managed Claude channels and the owned Codex bridge have bounded idle/busy/question acceptance; hooks alone cannot wake an idle model. See [message audit](MESSAGE-DELIVERY-AUDIT.md), [Codex input](CODEX-INPUT.md) and [Claude question evidence](verification/2026-09-12-claude-question-queue.json). |
...
| Next | Safe live daemon replacement | Child/process-group ownership, exact exit status, PTY/pipe I/O, logs, scrollback and identity now live in a per-agent session owner that outlives the daemon and is reattached on restart (schema 17); actual distinct-binary batch/PTY continuity acceptance is still to be recorded. Remaining: fence all writers/autostart; establish actual successor readiness and recover from failed transfer. Reconcile uncertain writes without blind replay. `daemon reload` remains unavailable. Event continuation, provider event reconnect, output drain and read-only queue retry are merged prerequisites, not completed replacement. See [replacement design](LIVE-DAEMON-UPGRADES.md). |
