...
| --- | --- | --- |
| User request, September 15 | Portable coordination instructions | One bundled SKILL.md now supplies MCP onboarding and preview/apply/undo setup for Codex, Claude Code and Gemini CLI; `agentdocker skill` exports it without a daemon. Profile overrides, generated-copy upgrades and preservation of custom/edited skills have focused coverage (31 setup/MCP tests passed). Actual installed Codex/Claude discovery passed in private profiles without model calls. Full integration/review, implicit model activation and additional runtime loader acceptance remain; the skill does not implement idle wake. See [guided setup](GUIDED-SETUP.md#shared-coordination-skill). |
