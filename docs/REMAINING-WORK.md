...
| --- | --- | --- |
| User request, September 15 | Portable coordination instructions | One bundled SKILL.md now supplies MCP onboarding and preview/apply/undo setup for Codex, Claude Code and Gemini CLI; `agentdocker skill` exports it without a daemon. Combined native-input source `d7bd90c` passed 969 Rust tests (six skipped), 70 Python checks and the full release gate, plus independent source review. Final release setup/undo and actual Codex/Claude discovery passed in private profiles without model calls; real Codex TUI acceptance also retained draft/FIFO/receiver recovery and queued human answers. Final CI, installation, implicit model activation and additional runtime loader acceptance remain; the skill alone does not implement idle wake. See [guided setup](GUIDED-SETUP.md#shared-coordination-skill). |
| Top | Install and verify native input for existing Codex terminals | The receiver connects the human/peer queue to Codex 0.154.0 native input with durable receipts and supervised recovery. Corrected `bb1e50c` passed 967 Rust tests (six skipped), 70 Python checks and the full release gate; final release-TUI acceptance at `b2c3938` passed idle wake, drafts, FIFO, receiver crash recovery and legacy human answers without replay. Earlier resume, rate-limit and schema20 migration trials remain recorded. Subsequent long-busy trial 26 at `5f72f37` exposed a false idle timeout during a direct user turn; #148/#149 are draft and merge/install are blocked on its fix and repeated actual acceptance. Final CI and installed-session verification remain pending. No-prompt fresh/reopened terminals still emit no SessionStart, and a 30-message burst took about five minutes; those gaps stay open. Other providers need their own adapters and acceptance. See [Codex input](CODEX-INPUT.md) and [source-specific evidence](verification/2026-09-15-native-codex-queue.json). |
