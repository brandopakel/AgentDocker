...
PR #168 Linux graphical CI found a stale acceptance expectation on both architectures: the contact-only fixture still waited for the old “Connected · idle delivery not verified” heading after the UI changed it to “Connected · messages wait for its next prompt”. The fixture now checks the current heading while retaining the separate unverified composer assertion. Both failed job logs are retained; the corrected actual-window trial and CI rerun remain pending.
