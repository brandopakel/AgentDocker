...
folder with Unicode, spaces and punctuation. The final combined gate at `932b566` passed 1,284 Rust tests (seven skipped),
94 Python checks (one skipped), formatting, strict lint, packaging and release.
The broader native workflow passed 545 steps and 31 checks across 12 windows;
final focus-reveal controls additionally passed the larger-text trial.
The clean `ce82d06` package passed 31 graphical checks across 12 windows and
13 previous/candidate installation and rollback scenarios, then activated as
`9e75e834`. UI process4324 uses that release; daemon92608 and receiver60959
remain on their retained releases. All eight live IDs and five provider PID/birth
pairs were preserved. macOS is locked, so the installed-window visual review
remains pending, as do final CI and review. External terminal hosts beyond macOS Terminal and native Linux shell
...
| Installed preview / review | Chat-first UX cleanup (September 18) | Combined PR #200 includes #201 naming and #203 session/Board cleanup. Shared Chat is first; project/agent terminals are visible; stable names, canonical DM grouping, temporary-project and system-feed folds, paged Earlier sessions, row-level reconnect and readable failures address the screenshot findings. Validation: final gate 1,284 Rust / seven skipped, 94 Python / one skipped; native workflow 545 steps / 31 checks / 12 windows; focused chat and larger-text trials plus a real project-folder Terminal action. Older alias histories, unread counts and drafts remain reachable. Clean source `ce82d06` is installed as `9e75e834` after 13 package-install checks and 31 packaged graphical checks. Final CI/review and an unlocked installed-window review remain; other terminal hosts and Linux native shell acceptance are not established. See the cleanup section above and the existing integrated desktop record. |
