...
| --- | --- | --- |
| Top | Existing idle sessions and provider parity | The current plain Claude session still has hooks/MCP without a channel input binding; peer messages can wait at its idle prompt. Install the reviewed #164 reconnect support, enable the supported channel route by a safe same-session relaunch, preserve original queued IDs/receipts and drafts, and prove a peer-only idle turn. New Claude/Codex UI launches have input defaults, but consent and actual session readiness still matter. Other supported runtimes need equivalent adapters and idle/busy/limit acceptance. [Message audit](MESSAGE-DELIVERY-AUDIT.md). |
| Top | Project pause and per-recipient delivery | The missed CLI/global and app/project pauses are reproduced and retained. Installed Codex bounded active input now passes, including real project fan-out; queue acceptance alone still does not prove every recipient paused. PR #173 implements durable project pause/resume, but its form needs project/request ownership, retry/draft preservation, stale-response protection and narrow-layout fixes. Validate the schema23-to22 rollback refusal and actual idle/busy recipients before closing it. |
...
| Engineering / acceptance | Setup and input readiness | Configuration, generation-bound contact and fresh receiver/receipt evidence are distinct. PR #168 adds named missing-hook diagnostics; its latest review requires malformed `disableAllHooks` values to stay Unverified and the guide to list StopFailure. Finish final review/CI, installation and actual-session setup acceptance. [Guided setup](GUIDED-SETUP.md). |
