...
| Acceptance | Complete notification release acceptance | AppleScript fallback removal, destination metadata and native routing are implemented. The intact installed `cf64ca3` bundle passes strict signature checks. With explicit user approval, macOS notification permission changed from denied to authorized; unchanged installed binaries then passed real foreground/background message clicks, pending-question navigation, stale-message fallback and another conversation's draft retention. A production-daemon send posted and routed end to end, and a cold private-origin window used the matching installed UI hash while retaining the production window. Owned fixtures were cleaned up. PR #158's diagnostic/first-focus changes are merged after final CI/review. Found on the installed machine on September 16: the required Claude Code hook set gained `StopFailure` on September 14 and every machine wired before that read **Needs setup** with no reason, indistinguishable from never having been set up; `runtimes` now reports `hooks_missing`, the Tools row and the CLI table say which event, and **Connected · idle delivery not verified** reads **Connected · messages wait for its next prompt** (a paused or silent receiver reads as such) so it is not taken to mean the session is idle. Malformed hook entry/matcher shapes now remain Unverified with no misleading list of missing events; a numeric matcher is never treated as an absent matcher on StopFailure. Source `50f1b0d` passed the focused malformed-matcher regression and the full 1,089-Rust/84-Python gate (seven skipped), formatting, strict lint, doctests, packaging and release build. Logs and JUnit are retained under `/private/tmp/agentdocker-hook-matcher-{targeted1,gate1}-2026-09-17`. Setup stays a person's action (it writes their provider configuration); what changed is that they are told what it will add. Remaining: zero-process app launch, old-notification and broader question/history/project cases, physical usability and Developer ID/notarized release acceptance. See [notification audit](NOTIFICATION-ROUTING-AUDIT.md). |
...
| Priority | Work remaining | Completion condition and evidence |
| --- | --- | --- |
| Partial | Messaging as a workspace (Slack/Discord shape) | PR #150 merged as `93b76b8` after final review and CI: schema21 archives, destination-scoped conversations, named channels, read cursors, unread counts, search, threads and the Messages workspace are implemented. Reading acknowledges only the person's displayed rows; agent input queues remain separate. The reviewed integration passed 999 Rust tests, 77 Python checks and 233 native workflow steps, with retained archive/index regression evidence in [session messages](verification/2026-09-11-session-messages.json). PR #152 adds the Earlier group, persistent project menus, saved names and corrected discovery; final source `2a51d13` passed 1007 Rust tests, 77 Python checks and 275 native steps. It merged as `ece76fd` after final CI/review. PR #160 pane resizing and hidden-conversation read-state protection merged as `37725fe` after final review and CI; widths remain saved, compact layouts preserve usable conversation space, and hidden panes do not acknowledge messages. Read-only September 16 verification observes source `1e90f83` installed as release `3add4bea`, with the daemon and GUI running from that release. PR #163 merged as `ad96698`: collision rooms retain their row counts but no longer inflate the person's unread badge or Mark all read. Broader installed acceptance remains. Enter sends in every composer; **+** beside the search opens a form for a new direct message (one pick) or a new channel (name, purpose, members; `channel_open` takes a `project` so a person can open one); `@` offers who is here and finishes the name, `conversations` counts the unread rows that mention the reader (`mentions`; `mentions_are_counted_for_the_reader_named`), rows show an **@n** pill and such messages say **mentions you**; the workflow smoke invokes the composer's submit action used by Enter, exercises the mention offer, creates a channel with a picked member, and creates a direct message. Physical keyboard acceptance remains separate. Still unbuilt: an explicit project-wide pause request/control. See [desktop behavior](DESKTOP-UX.md#messages-inbox-and-tools) and [integrated evidence](verification/2026-09-12-integrated-desktop.json). |
| Engineering | A board of work beside the sessions | Daemon side in source: cards as `task` documents with a title, acceptance text and a column; `task_pull` takes a Ready card once under the state lock (the second taker is told who holds it), `task_move` by the holder or the person (a move back to Backlog/Ready releases it), `task_update`, `task_archive`, `tasks`; five events; CLI `agentdocker task create|pull|move|update|archive|list`; MCP `list_tasks`, `pull_task`, `move_task`, `create_task`; the bundled skill tells agents to pull Ready cards and read the acceptance text first (`a_card_is_pulled_once_and_moved_by_its_holder_or_the_person`, core rules unit-tested). The shape follows the Paprika research in the [landscape note](LANDSCAPE-2026-09-11.md). The Board tab is in the app: five columns, filing a card as Ready or into Backlog, a card opened to its acceptance text and moved a column at a time, handed to an agent or released, archived; the holder shown with presence; the board read again on every board event. Still ahead: a pull refused while the project is paused (after #173), typed links, roles, webhooks, and the workflow smoke over the board. |
...
The public missing-event helper validates the same shape itself, including
malformed matchers and nested handlers, so direct callers receive the same
unknown-configuration result as the runtime inventory.
...
The full source gate passed for these two review corrections at `8d42db5`. PR #171 merged as `df3b758` after final review and all CI checks. The backed-up installed switch to candidate `9bc0f0fc` passed with schema22, unchanged live provider identities, receiver 94744 replaced by 20912, and all 58 earlier receipts plus the token/binding retained. Queued messages, a fresh self-canary, a clearly labeled CLI test through the human-input route, and a fresh Claude peer ping entered this same active Codex turn through the installed hook route. The CLI sample reached persisted provider context 0.176 seconds after send; this is one sample, not a latency guarantee. Exact receipts are recorded without manually acknowledging the queue. The actual app was closed during the switch and reopened afterward. The existing native-queue record contains the production evidence; other-provider, oversized-message and long-run acceptance remain open.

Reconnect integration retains the optional retired names added by the Messages review, including existing aliases flattened through another session fold. Missing legacy names remain unknown. Combined source `3c41b3b` passed the full 1,116-Rust/84-Python gate (seven skipped), formatting, strict lint, doctests, packaging and release. Its actual MCP resume adapter trial passed in 38.41 seconds with no fixture processes left. This does not prove idle wake for the current plain Claude session. Final review, CI and installed acceptance remain open.

The next Messages review preserves current and retired identity names for unread mentions. New aliases save the retired name; legacy aliases without it remain unknown. Four focused regressions and source `5c461e0` passed the full 1,108-Rust/84-Python gate (seven skipped), lint, doctests, packaging and release. The invitation regression covers both stale success and failure. The earlier native workflow covers the unchanged composer controls; final graphical CI, review and installation remain pending.

Named hook diagnostics combined with the Messages controls at `c757177` passed 1,110 Rust tests, 84 Python checks, formatting, strict lint, doctests, packaging and release (seven Rust tests skipped), followed by 415 rendered native workflow steps and 26 outer checks in 132 seconds. The actual provider configurations were not changed by this isolated trial. Final review, CI and installed acceptance remain open.

PR #168 final hook-type follow-up rejects non-Boolean `disableAllHooks` values as Unverified, with no misleading missing-event list. The guide includes StopFailure. Combined `1cdec50` includes merged session reconnect #164 and passed 1,118 Rust tests (seven skipped), 84 Python checks and the full formatting/lint/doctest/package/release gate. Prior rendered workflow evidence covers the unchanged controls; physical keyboard and installed diagnostics acceptance remain.
