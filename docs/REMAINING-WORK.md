...
| Engineering / acceptance | Reply from the notification | macOS message notifications are posted under a category with a `UNTextInputNotificationAction` (**Reply**, registered by the poster and by the window); the typed reply comes back to the app's notification delegate, which sends it through the daemon the notification names — from `user`, to the channel or the agent, `reply_to` the message, so an answer closes its question (`a_reply_answers_the_notifications_conversation_as_the_person`) — off the delegate's thread and without opening a window; a failed send is said as a notification. Not established: a physical Notification Center reply on this Mac (the same authorization boundary as the click routing above). |
