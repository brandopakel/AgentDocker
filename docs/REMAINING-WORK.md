...
| Implemented; native acceptance pending | Terminal selection and richer interaction | Range selection/copy now retains a bounded visible-grid snapshot, preserves Unicode and wrapped text, and releases the snapshot after copy or resumed input. Finish native/physical selection, keyboard focus and input-method trials, and repair observed usability/accessibility defects. | [Iced contracts](ICED-DESIGN.md), [desktop guide](DESKTOP-UX.md) |
